...
namespace Kadath
{
    ...
    Metric_nophi_AADS_const::Metric_nophi_AADS_const(Metric_tensor& met, const Scalar& conf)
        : Metric_nophi_AADS(met, conf)
    {
    }

    Metric_nophi_AADS_const::Metric_nophi_AADS_const(const Metric_nophi_AADS_const& so)
        : Metric_nophi_AADS(*so.p_met, so.conformal)
    {
    }

    Metric_nophi_AADS_const::~Metric_nophi_AADS_const() {}

    void Metric_nophi_AADS_const::set_system(System_of_eqs& ss, const char* name_met)
    {
    ...
    void Metric_nophi_AADS_const::compute_cov(int dd) const
    {
    ...
    void Metric_nophi_AADS_const::compute_con(int dd) const
    {
        int dim = espace.get_ndim();
        if (dim != 3) {
            KADATH_THROW("Function only implemented for dimension 3");
