...
namespace Kadath
{
    Metric_dirac_const::Metric_dirac_const(Metric_tensor& met) : Metric_dirac(met) {}

    Metric_dirac_const::Metric_dirac_const(const Metric_dirac_const& so) : Metric_dirac(*so.p_met) {}

    Metric_dirac_const::~Metric_dirac_const() {}

    void Metric_dirac_const::set_system(System_of_eqs& ss, const char* name_met)
    {
    ...
    void Metric_dirac_const::compute_cov(int dd) const
    {
    ...
    void Metric_dirac_const::compute_con(int dd) const
    {
        int dim = espace.get_ndim();
        if (dim != 3) {
            KADATH_THROW("Function only implemented for dimension 3");
