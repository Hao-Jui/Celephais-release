...
namespace Kadath
{
    Ope_ddp::Ope_ddp(const System_of_eqs* zesys, Ope_eq* target) : Ope_eq(zesys, target->get_dom(), 1)
    {
    ...
    Term_eq Ope_ddp::action() const
    {
        ...
        return ope_tensor_unary_lane_detail::apply_tensor_unary_operator(
            dom, target, "Ope_ddp", [](const Val_domain& value) { return value.get_domain()->ddp(value); });
    }
} // namespace Kadath
