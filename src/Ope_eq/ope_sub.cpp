...
namespace Kadath
{
    Ope_sub::Ope_sub(const System_of_eqs* zesys, Ope_eq* aa, Ope_eq* bb) : Ope_eq(zesys, aa->get_dom(), 2)
    {
    ...
    bool Ope_sub::action_into(Term_eq& result) const
    {
        ScopedOpeActionProfile ope_action_profile_scope(*this);
        ope_action_detail::OperandScratchLease lhs_scratch(syst);
        const Term_eq& lhs = parts[0]->action_operand(lhs_scratch.storage());
        ope_action_detail::OperandScratchLease rhs_scratch(syst);
        const Term_eq& rhs = parts[1]->action_operand(rhs_scratch.storage());
        if (result.try_write_sum_action_result(lhs, rhs, true))
            return true;
    ...
    void Ope_sub::action_into_scratch(std::optional<Term_eq>& result) const
    {
        ScopedOpeActionProfile ope_action_profile_scope(*this);
        ope_action_detail::OperandScratchLease lhs_scratch(syst);
        const Term_eq& lhs = parts[0]->action_operand(lhs_scratch.storage());
        ope_action_detail::OperandScratchLease rhs_scratch(syst);
        const Term_eq& rhs = parts[1]->action_operand(rhs_scratch.storage());
        if (result.has_value() && result->try_write_sum_action_result(lhs, rhs, true))
            return;
        if (!result.has_value()) {
