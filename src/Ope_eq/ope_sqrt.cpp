...
namespace Kadath
{
    Ope_sqrt::Ope_sqrt(const System_of_eqs* zesys, Ope_eq* target) : Ope_eq(zesys, target->get_dom(), 1)
    {
    ...
    Term_eq Ope_sqrt::action() const
    {
        ...
        switch (target.type_data) {
            case TERM_T: {
                ...
                if (target.val_t->get_valence() != 0) {
                    KADATH_THROW("Ope_sqrt only defined with respect for a scalar");
                }
        ...
        KADATH_THROW("Warning should not be here in Ope_sqrt::action...");
    }
} // namespace Kadath
