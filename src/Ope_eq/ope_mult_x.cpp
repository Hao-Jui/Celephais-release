...
namespace Kadath
{
    Ope_mult_x::Ope_mult_x(const System_of_eqs* zesys, Ope_eq* target) : Ope_eq(zesys, target->get_dom(), 1)
    {
        parts[0].reset(target);
    }

    Ope_mult_x::~Ope_mult_x() {}

    Term_eq Ope_mult_x::action() const
    {
        ...
        return ope_tensor_unary_lane_detail::apply_tensor_unary_operator(
            dom, target, "Ope_mult_x", [](const Val_domain& value) { return value.get_domain()->mult_x(value); });
    }
} // namespace Kadath
