...
namespace Kadath
{
    Ope_der_background::Ope_der_background(const System_of_eqs* zesys, int tt, char ind, Ope_eq* target)
    ...
    Ope_der_background::~Ope_der_background() {}

    Term_eq Ope_der_background::action() const
    {
        ...
        if (target.type_data != TERM_T) {
            KADATH_THROW("Ope_der_background only defined with respect for a tensor");
        }

        return (syst->get_met()->get_background()->derive(type_der, ind_der, target));
    }
} // namespace Kadath
