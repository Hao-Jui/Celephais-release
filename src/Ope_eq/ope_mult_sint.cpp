...
namespace Kadath
{
    Ope_mult_sint::Ope_mult_sint(const System_of_eqs* zesys, Ope_eq* target) : Ope_eq(zesys, target->get_dom(), 1)
    {
        parts[0].reset(target);
    }

    Ope_mult_sint::~Ope_mult_sint() {}

    Term_eq Ope_mult_sint::action() const
    {
