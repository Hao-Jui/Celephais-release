...
namespace Kadath
{
    Ope_srdr::Ope_srdr(const System_of_eqs* zesys, Ope_eq* target) : Ope_eq(zesys, target->get_dom(), 1)
    {
    ...
    Term_eq Ope_srdr::action() const
    {
        ...
        return ope_tensor_unary_lane_detail::apply_tensor_unary_operator(
            dom, target, "Ope_srdr", [](const Val_domain& value) { return value.get_domain()->srdr(value); });
    }
} // namespace Kadath
