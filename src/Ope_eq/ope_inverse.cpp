...
namespace Kadath
{
    ...
    Term_eq Ope_inverse::action() const
    {
        ...
        if (dim != 3) {
            KADATH_THROW("Ope_inverse currently supports only 3D tensors");
        }
        ...
        do {
            ...
            detval += signature * (*target.val_t)(p1, 1)(dom) * (*target.val_t)(p2, 2)(dom) *
                      (*target.val_t)(p3, 3)(dom); // sum on permutations
            if (m_doder) {
                detder += signature *
                          ((*target.der_t)(p1, 1)(dom) * (*target.val_t)(p2, 2)(dom) * (*target.val_t)(p3, 3)(dom) +
