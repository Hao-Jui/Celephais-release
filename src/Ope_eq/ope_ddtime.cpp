...
namespace Kadath
{
    Ope_ddtime::Ope_ddtime(const System_of_eqs* zesys, Ope_eq* target) : Ope_eq(zesys, target->get_dom(), 1)
    {
        parts[0].reset(target);
    }

    Ope_ddtime::~Ope_ddtime() {}

    Term_eq Ope_ddtime::action() const
    {
        ...
        return ope_tensor_unary_lane_detail::apply_tensor_unary_operator(
            dom, target, "Ope_ddtime",
            [](const Val_domain& value) { return value.get_domain()->ddtime(value); });
    }
} // namespace Kadath
