...
namespace Kadath
{
    Ope_div_rsint::Ope_div_rsint(const System_of_eqs* zesys, Ope_eq* target) : Ope_eq(zesys, target->get_dom(), 1)
    {
        parts[0].reset(target);
    }

    Ope_div_rsint::~Ope_div_rsint() {}

    Term_eq Ope_div_rsint::action() const
    {
        ...
        if (part.type_data != TERM_T) {
            KADATH_THROW("Ope_div_rsint only defined with respect to a tensor");
        }
        ...
        return ope_tensor_unary_lane_detail::apply_tensor_unary_operator(
            dom, target, "Ope_div_rsint",
            [](const Val_domain& value) { return value.get_domain()->div_sin_theta(value); });
    }
} // namespace Kadath
