...
namespace Kadath
{
    Ope_ddt::Ope_ddt(const System_of_eqs* zesys, Ope_eq* target) : Ope_eq(zesys, target->get_dom(), 1)
    {
    ...
    Term_eq Ope_ddt::action() const
    {
        ...
        return ope_tensor_unary_lane_detail::apply_tensor_unary_operator(
            dom, target, "Ope_ddt", [](const Val_domain& value) { return value.get_domain()->ddt(value); });
    }
} // namespace Kadath
