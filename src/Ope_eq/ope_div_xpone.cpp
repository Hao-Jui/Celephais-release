...
namespace Kadath
{
    Ope_div_xpone::Ope_div_xpone(const System_of_eqs* zesys, Ope_eq* target) : Ope_eq(zesys, target->get_dom(), 1)
    {
        parts[0].reset(target);
    }

    Ope_div_xpone::~Ope_div_xpone() {}
    ...
    Term_eq Ope_div_xpone::action() const
    {
        ...
        return ope_tensor_unary_lane_detail::apply_tensor_unary_operator(
            dom, target, "Ope_div_xpone",
            [](const Val_domain& value) { return value.get_domain()->div_xp1(value); });
    }
} // namespace Kadath
