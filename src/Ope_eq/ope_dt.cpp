...
namespace Kadath
{
    Ope_dt::Ope_dt(const System_of_eqs* zesys, Ope_eq* target) : Ope_eq(zesys, target->get_dom(), 1)
    {
    ...
    Term_eq Ope_dt::action() const
    {
        ...
        return ope_tensor_unary_lane_detail::apply_tensor_unary_operator(
            dom, target, "Ope_dt", [](const Val_domain& value) { return value.get_domain()->dt(value); });
    }
} // namespace Kadath
