...
namespace Kadath
{
    Ope_div_1mx2::Ope_div_1mx2(const System_of_eqs* zesys, Ope_eq* target) : Ope_eq(zesys, target->get_dom(), 1)
    {
        parts[0].reset(target);
    }

    Ope_div_1mx2::~Ope_div_1mx2() {}

    Term_eq Ope_div_1mx2::action() const
    {
        ...
        return ope_tensor_unary_lane_detail::apply_tensor_unary_operator(
            dom, target, "Ope_div_1mx2",
            [](const Val_domain& value) { return value.get_domain()->div_1mx2(value); });
    }
} // namespace Kadath
