...
namespace Kadath
{
    Term_eq Domain_shell_inner_homothetic_nosym::integ_term_eq(const Term_eq& so, int bound) const
    {
        ...
        { // The value
            ...
            if (value.check_if_zero())
                ...
                switch (baset) {
                    ...
                    default:
                        KADATH_THROW("Case not yet implemented in Domain_shell_inner_homothetic_nosym::integrale");
                }
            }
            resval *= 2 * M_PI;
        }
