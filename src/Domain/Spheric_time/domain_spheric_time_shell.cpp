...
namespace Kadath
{
    ...
    Domain_spheric_time_shell::Domain_spheric_time_shell(int num, int ttype, double tmmin, double tmmax, double rrmin,
                                                         double rrmax, const Dim_array& nbr)
    ...
    // Constructor by copy
    Domain_spheric_time_shell::Domain_spheric_time_shell(const Domain_spheric_time_shell& so)
        : Domain(so), alpha(so.alpha), beta(so.beta), tmin(so.tmin), tmax(so.tmax)
    {
    }

    Domain_spheric_time_shell::Domain_spheric_time_shell(int num, BinarySource& source) : Domain(num, source)
    {
    ...
    void Domain_spheric_time_shell::save(BinarySink& sink) const
    {
    ...
    Val_domain Domain_spheric_time_shell::der_normal(const Val_domain& so, int bound) const
    {
        ...
        switch (bound) {
            ...
            default:
                KADATH_THROW("Unknown boundary case in Domain_spheric_time_shell::der_normal");
        }
    }
    ...
    void Domain_spheric_time_shell::do_coloc()
    {
        ...
        switch (type_base) {
            ...
            case LEG_TYPE:
                ...
                break;
            default:
                KADATH_THROW("Unknown type of basis in Domain_spheric_time_shell::do_coloc");
        }
    }
