...
namespace Kadath {
void Domain_bispheric_rect_nosym::export_tau_val_domain_boundary_one_side (const Val_domain& so, int bound, Array<double>& sec, int& pos_sec, int ncond) const {
						...
						if ((k==0) || (k%2==1)) {
							...
							switch (type_base) {
								...
								case LEG_TYPE :
									...
									break ;
								default :
					cerr << "Unknown type of basis in Domain_bispheric_rect_nosym::export_tau_boundary_one_side" << endl ;
										abort() ;
								}
						if (!anchor_ready) {
							pos_galerkin = pos ;
							pos_galerkin.set(1) = 0 ;
							anchor_boundary = val_boundary (bound, so, pos_galerkin) ;
							anchor_ready = true ;
						}
						sec.set(pos_sec) = val_boundary (bound, so, pos) +
								factor_galerkin * anchor_boundary ;
						pos_sec ++ ;
						}}
						break ;
					case SIN :
						...
						break ;
					default :
						cerr << "Unknown base in Domain_bispheric_rect_nosym::export_tau_boundary_one_side" << endl ;
					break ;
				}
			}
		}
			...
			switch (basep) {
				case COS :
						...
						if ((k==0) || (k%2==1)) {
							...
							switch (type_base) {
								...
								case LEG_TYPE :
									...
									break ;
								default :
					cerr << "Unknown type of basis in Domain_bispheric_rect_nosym::export_tau_val_domain_boundary_one_side_one_side" << endl ;
										abort() ;
								}
						if (!anchor_ready) {
							pos_galerkin = pos ;
					...
					case SIN :
							...
							if (k%2==1) {
								...
								switch (type_base) {
									...
									case LEG_TYPE :
										...
										break ;
									default :
						cerr << "Unknown type of basis in Domain_bispheric_rect_nosym::export_tau_val_domain_boundary_one_side" << endl ;
										abort() ;
								}
							if (!anchor_ready) {
								pos_galerkin = pos ;
						...
						break ;
					default :
						cerr << "Unknown base in Domain_bispheric_rect_nosym::export_tau_val_domain_boundary_one_side" << endl ;
					break ;
				}
			}
		}
...
void Domain_bispheric_rect_nosym::export_tau_boundary_one_side (const Tensor& tt, int dom, int bound, Array<double>& res, int& pos_res, const Array<int>& ncond,
										int n_cmp, Array<int>** p_cmp) const {

	// Check boundary
	if ((bound!=INNER_BC) && (bound!=ETA_PLUS_BC) && (bound!=CHI_ONE_BC) && (bound!=OUTER_BC)) {
		cerr << "Unknown boundary in Domain_bispheric_rect_nosym::export_tau_boundary_one_side" << endl ;
		abort() ;
	}
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_rect_nosym::export_tau_boundary_one_side" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_rect_nosym::export_tau_boundary_one_side" << endl ;
				abort() ;
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_bispheric_rect_nosym::export_tau_boundary_one_side" << endl ;
			break ;
	}
}}
