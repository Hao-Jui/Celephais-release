...
namespace Kadath {
int Domain_bispheric_rect_nosym::nbr_conditions_val_domain_boundary_one_side (const Val_domain& so, int bound) const {
	...
	for (int k=0 ; k<nbr_coefs(2) ; k++) {
		...
		for (int j=0 ; j<nbr_coefs(1) ; j++) {
			...
			switch (basep) {
			  ...
			  break ;
		      default :
			  cerr << "Unknwon phi basis in Domain_bispheric_rect_nosym::nbr_conditions_val_boundary" << endl ;
			  abort() ;
		      }
	...
	if (bound==ETA_PLUS_BC) {
		...
		case SIN :
			...
			break ;
		default :
			cerr << "Unknwon phi basis in Domain_bispheric_rect_nosym::nbr_conditions_vall_boundary" << endl ;
			abort() ;
		}
		if (true_other)
			res ++ ;
	...
	if (bound==CHI_ONE_BC)
		res = (basep==COS) ? nbr_coefs(2)*(nbr_coefs(0)-1) : (nbr_coefs(2)-2)*(nbr_coefs(0)-1) ;
...
Array<int> Domain_bispheric_rect_nosym::nbr_conditions_boundary_one_side (const Tensor& tt, int dom, int bound, int n_cmp, Array<int>** p_cmp) const {

	// Check boundary
	if ((bound!=INNER_BC) && (bound!=CHI_ONE_BC) && (bound!=ETA_PLUS_BC) && (bound!=OUTER_BC)) {
		cerr << "Unknown boundary in Domain_bispheric_rect_nosym::nbr_conditions_boundary_one_side" << endl ;
		abort() ;
	}
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_rect_nosym::nbr_conditions_boundary_one_side" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_rect_nosym::nbr_conditions_boundary_one_side" << endl ;
				abort() ;
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_bispheric_rect_nosym::nbr_conditions_boundary_one_side" << endl ;
			break ;
	}
	return res ;
}}
