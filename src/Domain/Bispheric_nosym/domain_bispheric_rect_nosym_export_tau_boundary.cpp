...
namespace Kadath {
void Domain_bispheric_rect_nosym::export_tau_val_domain_boundary (const Val_domain& so, int bound, Array<double>& sec, int& pos_sec, int ncond) const {
...
void Domain_bispheric_rect_nosym::export_tau_boundary (const Tensor& tt, int dom, int bound, Array<double>& res, int& pos_res, const Array<int>& ncond,
										int n_cmp, Array<int>** p_cmp) const {

	// Check boundary
	if ((bound!=INNER_BC) && (bound!=ETA_PLUS_BC) && (bound!=CHI_ONE_BC) && (bound!=OUTER_BC)) {
		cerr << "Unknown boundary in Domain_bispheric_rect_nosym::export_tau_boundary" << endl ;
		abort() ;
	}
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_rect_nosym::export_tau_boundary" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_rect_nosym::export_tau_boundary" << endl ;
				abort() ;
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_bispheric_rect_nosym::export_tau_boundary" << endl ;
			break ;
	}
}}
