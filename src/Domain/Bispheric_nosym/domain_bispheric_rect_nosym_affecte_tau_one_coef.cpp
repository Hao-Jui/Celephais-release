...
namespace Kadath {
void Domain_bispheric_rect_nosym::affecte_tau_one_coef_val_domain (Val_domain& so,  int cc, int& conte) const {
	...
	do {
		...
		if (indic) {
			if (conte==cc) {
				found = true ;
				so.cf->set(pos_cf) = 1;
				// Regularity on the axis when needed :
				if (((basechi==CHEB_EVEN) || (basechi==LEG_EVEN)) && (pos_cf(2)!=0)) {
					...
					switch (basechi) {
						...
						case LEG_EVEN :
							...
							break ;
						default :
							cerr << "Unknown base in Domain_bispheric_rect_nosym::affecte_one_coef" << endl ;
							abort() ;
						}
...
void Domain_bispheric_rect_nosym::affecte_tau_one_coef (Tensor& tt, int dom, int cc, int& pos_cf) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_rect_nosym::affecte_tau_one_coef" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_rect_nosym::affecte_tau_one_coef" << endl ;
				abort() ;
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_bispheric_rect_nosym::affecte_tau" << endl ;
			break ;
	}
}

bool Domain_bispheric_rect_nosym::describe_tau_seed_block(
