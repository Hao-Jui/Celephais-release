...
namespace Kadath {
int Domain_bispheric_eta_first_nosym::nbr_unknowns_val_domain (const Val_domain& so) const {
...
int Domain_bispheric_eta_first_nosym::nbr_unknowns (const Tensor& tt, int dom) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_eta_first_nosym::nbr_unknowns" << endl ;
				abort() ;
			}
		}
		    break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_eta_first_nosym::nbr_unknowns" << endl ;
				abort() ;
			}
		}
		break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_bispheric_eta_first_nosym::nbr_unknowns" << endl ;
			break ;
	}
	return res ;
}
