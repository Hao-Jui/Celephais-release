...
namespace Kadath {
int Domain_bispheric_chi_first_nosym::nbr_conditions_val_domain_boundary_one_side (const Val_domain& so, int bound) const {
	...
	for (int k=0 ; k<nbr_coefs(2) ; k++) {
		...
		for (int j=0 ; j<nbr_coefs(1) ; j++) {
			...
			switch (basep) {
			  ...
			  break ;
		      default :
			  cerr << "Unknwon phi basis in Domain_bispheric_chi_first_nosym::nbr_conditions_val_boundary_one_side" << endl ;
			  abort() ;
		      }
...
Array<int> Domain_bispheric_chi_first_nosym::nbr_conditions_boundary_one_side (const Tensor& tt, int dom, int bound, int n_cmp, Array<int>** p_cmp) const {

	// Check boundary
	if ((bound!=INNER_BC) && (bound!=CHI_ONE_BC) && (bound!=OUTER_BC)) {
		cerr << "Unknown boundary in Domain_bispheric_chi_first_nosym::nbr_conditions_boundary_one_side" << endl ;
		abort() ;
	}
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_chi_first_nosym::nbr_conditions_boundary" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_chi_first_nosym::nbr_conditions_boundary_one_side" << endl ;
				abort() ;
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_bispheric_chi_first_nosym::nbr_conditions_boundary_one_side" << endl ;
			break ;
	}
	return res ;
}
