...
namespace Kadath {
void Domain_bispheric_chi_first_nosym::export_tau_val_domain (const Val_domain& so, int order, Array<double>& sec, int& pos_sec, int ncond) const {
	...
	switch (order) {
	  ...
	  case 2 :
	      ...
	      break ;
	  default:
	      cerr << "Unknown order in Domain_bispheric_chi_first_nosym:export_tau_val_domain" << endl ;
	      break ;
	}
...
void Domain_bispheric_chi_first_nosym::export_tau (const Tensor& tt, int dom, int order, Array<double>& res, int& pos_res, const Array<int>& ncond,
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_chi_first_nosym::export_tau" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_chi_first_nosym::export_tau" << endl ;
				abort() ;
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_bispheric_chi_first_nosym::export_tau" << endl ;
			break ;
	}
}}
