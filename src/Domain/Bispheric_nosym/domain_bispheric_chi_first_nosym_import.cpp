...
namespace Kadath {
// Tensorial parts :
Tensor Domain_bispheric_chi_first_nosym::import (int numdom, int bound, int n_ope, const Array<int>& zedoms, Tensor** parts) const {
  ...
  switch (bound) {
    ...
    case OUTER_BC :
      ...
      break ;
    default :
      cerr << "Unknown boundary in Domain_bispheric_chi_first_nosym::import" << endl ;
  }
  ...
  for (int k=0 ; k<nbr_points(2) ; k++)
	...
	if (parts[0]->get_space().get_domain(zedoms(current))->is_in(MM, 1e-3))
      ...
      if (!found) {
	cerr << "Point " << MM << " not found in other domains, for Domain_bispheric_chi_first_nosym::import" << endl ;
	abort() ;
      }
