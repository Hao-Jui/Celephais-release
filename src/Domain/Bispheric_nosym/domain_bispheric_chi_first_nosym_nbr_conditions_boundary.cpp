...
namespace Kadath {
int Domain_bispheric_chi_first_nosym::nbr_conditions_val_domain_boundary (const Val_domain& so, int bound) const {
...
Array<int> Domain_bispheric_chi_first_nosym::nbr_conditions_boundary (const Tensor& tt, int dom, int bound, int n_cmp, Array<int>** p_cmp) const {

	// Check boundary
	if ((bound!=INNER_BC) && (bound!=CHI_ONE_BC) && (bound!=OUTER_BC)) {
		cerr << "Unknown boundary in Domain_bispheric_chi_first_nosym::nbr_conditions_boundary" << endl ;
		abort() ;
	}
	...
	switch (val) {
		...
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_chi_first_nosym::nbr_conditions_boundary" << endl ;
				abort() ;
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_bispheric_chi_first_nosym::nbr_conditions_boundary" << endl ;
			break ;
	}
	return res ;
}

bool Domain_bispheric_chi_first_nosym::describe_boundary_residual_rows(
