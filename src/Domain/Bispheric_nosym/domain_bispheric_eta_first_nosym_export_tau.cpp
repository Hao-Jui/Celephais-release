...
namespace Kadath {
void Domain_bispheric_eta_first_nosym::export_tau_val_domain (const Val_domain& so, int order, Array<double>& sec, int& pos_sec, int ncond) const {
	...
	switch (order) {
	  ...
	  case 2 :
	      ...
	      break ;
	  default:
	      cerr << "Unknown order in Domain_bispheric_eta_first_nosym::export_tau_val_domain" << endl ;
	      break ;
	}
	...
	if (k!=1) {
		...
		for (int j=0 ; j<nbr_coefs(1)-forgot_eta ; j++) {
			...
			// bound for i
			int maxi = nbr_coefs(0)-forgot_chi ;
			// case for odd
					...
					if (k==0) {
						...
						switch (type_base) {
							...
							case LEG_TYPE :
								...
								break ;
							default :
						cerr << "Unknown type of basis in Domain_bispheric_eta_first_nosym::export_tau_val_domain" << endl ;
										abort() ;
								}
							const std::size_t anchor_offset = row_offset ;
							result_data[pos_sec] = coefficient_data[coefficient_offset] +
...
void Domain_bispheric_eta_first_nosym::export_tau (const Tensor& tt, int dom, int order, Array<double>& res, int& pos_res, const Array<int>& ncond,
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_eta_first_nosym::export_tau" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_eta_first_nosym::export_tau" << endl ;
				abort() ;
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_bispheric_eta_first_nosym::export_tau" << endl ;
			break ;
	}
}
}
