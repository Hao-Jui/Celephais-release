...
namespace Kadath {
int Domain_bispheric_eta_first_nosym::nbr_conditions_val_domain (const Val_domain& so, int order) const {
	...
	switch (order) {
	  ...
	  case 2 :
	      ...
	      break ;
	  default:
	      cerr << "Unknown order in Domain_bispheric_eta_first_nosym::nbr_conditions_val_domain" << endl ;
	      break ;
	}
	...
	for (int k=0 ; k<nbr_coefs(2) ; k++)
		// loop on eta
		for (int j=0 ; j<nbr_coefs(1)-forgot_eta ; j++)
			// Loop on chi ;
			for (int i=0 ; i<nbr_coefs(0) ; i++) {
				...
				// No last coef if odd
				if (((basechi==CHEB_ODD) || (basechi==LEG_ODD)) && (i>=nbr_coefs(0)-1-forgot_chi))
					true_other = false ;

				//Galerkin for even ones
				if (((basechi==CHEB_EVEN) || (basechi==LEG_EVEN)) && (k!=0) && (i==0))
					true_other = false ;

				if (((basechi==CHEB_EVEN) || (basechi==LEG_EVEN)) && (i>=nbr_coefs(0)-forgot_chi))
					true_other = false ;
...
Array<int> Domain_bispheric_eta_first_nosym::nbr_conditions (const Tensor& tt, int dom, int order, int n_cmp, Array<int>** p_cmp) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_eta_first_nosym::nbr_conditions" << endl ;
				abort() ;
			}
		}
		      break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_eta_first_nosym::nbr_conditions" << endl ;
				abort() ;
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_bispheric_eta_first_nosym::nbr_conditions" << endl ;
			break ;
	}
	return res ;
}

bool Domain_bispheric_eta_first_nosym::describe_volume_residual_rows(
    ...
    Array<int>** p_cmp, std::vector<ResidualRowDescriptor>& descriptors) const
{
    ...
    if (!residual_tensor_components_in_tau_order(
        ...
        ncond.get_nbr() != components.size() || order < 0 || order > 2 ||
        (tt.get_valence() > 0 &&
         tt.get_basis().get_basis(dom) != CARTESIAN_BASIS)) {
        return false;
    }
    const int forgot_chi = order == 0 ? 0 : 1;
    const int forgot_eta = order == 0 ? 0 : 2;
    ...
    for (std::size_t slot = 0; slot < components.size(); ++slot) {
        ...
        for (int k = 0; k < nbr_coefs(2) - 1; ++k) {
            if (k == 1)
                continue;
            for (int j = 0; j < nbr_coefs(1) - forgot_eta; ++j) {
                const int chi_basis = (*field.get_base().bases_1d[0])(j, k);
                ...
                int max_chi = nbr_coefs(0) - forgot_chi;
                if (odd)
                    --max_chi;
