...
namespace Kadath {
void Domain_bispheric_rect_nosym::affecte_tau_val_domain (Val_domain& so, const Array<double>& values, int& conte) const {
	...
	*so.cf = 0. ;
	...
	// Loop on phi when needed (i.e. when Galerkin base is used) :
					...
					for (int j=1 ; j<nbr_coefs(1) ; j++) {
						...
						switch (basechi) {
							...
							case LEG_EVEN :
								...
								break ;
							default :
								cerr << "Unknown base in Domain_bispheric_rect_nosym::affecte_tau" << endl ;
								abort() ;
						}
					}
					pos.set(1) = 0 ;
...
void Domain_bispheric_rect_nosym::affecte_tau (Tensor& tt, int dom, const Array<double>& cf, int& pos_cf) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_rect_nosym::affecte_tau" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_rect_nosym::affecte_tau" << endl ;
				abort() ;
			}
		}
			break ;
