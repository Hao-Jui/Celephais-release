...
namespace Kadath {
// Tensorial parts :
Tensor Domain_bispheric_rect_nosym::import (int numdom, int bound, int n_ope, const Array<int>& zedoms, Tensor** parts) const {
  ...
  if (bound==INNER_BC) {
	...
	while ((current<n_ope) && (!found)) {
      ...
      if (!found) {
	cerr << "Point " << MM << " not found in other domains, for Domain_bispheric_rect_nosym::import" << endl ;
	abort() ;
      }
  ...
  cerr << "unknown bound in Domain_bispheric_rect_nosym::import" << endl ;
  abort() ;
}}
