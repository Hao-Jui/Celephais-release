...
namespace Kadath {
int Domain_bispheric_rect_nosym::nbr_conditions_val_domain (const Val_domain& so, int order) const {
	...
	switch (order) {
	  ...
	  case 2 :
	      ...
	      break ;
	  default:
	      cerr << "Unknown order in Domain_bispheric_rect_nosym::nbr_conditions_val_domain" << endl ;
	      abort() ;
	}
...
Array<int> Domain_bispheric_rect_nosym::nbr_conditions (const Tensor& tt, int dom, int order, int n_cmp, Array<int>** p_cmp) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_rect_nosym::nbr_conditions" << endl ;
				abort() ;
			}
		}
		      break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_rect_nosym::nbr_conditions" << endl ;
				abort() ;
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_bispheric_rect_nosym::nbr_conditions" << endl ;
			break ;
	}
	return res ;
}

bool Domain_bispheric_rect_nosym::describe_volume_residual_rows(
