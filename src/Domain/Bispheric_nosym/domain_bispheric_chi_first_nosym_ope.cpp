...
Val_domain Domain_bispheric_chi_first_nosym::mult_cos_phi (const Val_domain& so) const {
	so.coef() ;
	Val_domain res(this) ;
	...
	do {
		switch ((*so.base.bases_1d[1])(index)) {
			...
			case LEG_EVEN :
				...
				break ;
				default :
					cout << "Unknown case in Domain_bispheric_chi_first_nosym::base_mult_sin_phi" << endl ;
					abort() ;
				}
		}
	while (index.inc()) ;
...
Val_domain Domain_bispheric_chi_first_nosym::mult_sin_phi (const Val_domain& so) const {
	so.coef() ;
	Val_domain res(this) ;
	...
	do {
		switch ((*so.base.bases_1d[1])(index)) {
			...
			case LEG_EVEN :
				...
				break ;
				default :
					cout << "Unknown case in Domain_bispheric_chi_first_nosym::base_mult_cos_phi" << endl ;
					abort() ;
				}
		}
	while (index.inc()) ;
...
Val_domain Domain_bispheric_chi_first_nosym::div_chi (const Val_domain& so) const {
	so.coef() ;
	Val_domain res(this) ;
...
Val_domain Domain_bispheric_chi_first_nosym::div_sin_chi(const Val_domain& so) const {
	...
	do {
		if (index(1)==0)
			res.set(index) = copie(index)/chi_max ;
	}
	while (index.inc()) ;
...
Val_domain Domain_bispheric_chi_first_nosym::mult_r (const Val_domain& so) const {
