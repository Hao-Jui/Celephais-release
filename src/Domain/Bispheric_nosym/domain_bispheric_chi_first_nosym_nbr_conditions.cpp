...
namespace Kadath {
int Domain_bispheric_chi_first_nosym::nbr_conditions_val_domain (const Val_domain& so, int order) const {
	...
	switch (order) {
	  ...
	  case 2 :
	      ...
	      break ;
	  default:
	      cerr << "Unknown order in Domain_bispheric_chi_first_nosym::nbr_conditons_val_domain" << endl ;
	      abort() ;
	}
	...
	for (int k=0 ; k<nbr_coefs(2) ; k++)
		for (int j=0 ; j<nbr_coefs(1) ; j++) {
			...
			// No last coef if odd
			if (((basechi==CHEB_ODD) || (basechi==LEG_ODD)) && (j>=nbr_coefs(1)-1-forgot_chi))
				true_other = false ;
			...
			if (((basechi==CHEB_EVEN) || (basechi==LEG_EVEN)) && (j>=nbr_coefs(1)-forgot_chi))
				true_other = false ;
...
Array<int> Domain_bispheric_chi_first_nosym::nbr_conditions (const Tensor& tt, int dom, int order, int n_cmp, Array<int>** p_cmp) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_chi_first_nosym::nbr_conditions" << endl ;
				abort() ;
			}
		}
		      break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_chi_first_nosym::nbr_conditions" << endl ;
				abort() ;
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_bispheric_chi_first_nosym::nbr_conditions" << endl ;
			break ;
	}
	return res ;
}

bool Domain_bispheric_chi_first_nosym::describe_volume_residual_rows(
    ...
    Array<int>** p_cmp, std::vector<ResidualRowDescriptor>& descriptors) const
{
    ...
    for (std::size_t slot = 0; slot < components.size(); ++slot) {
        ...
        for (int k = 0; k < nbr_coefs(2) - 1; ++k) {
            ...
            int max_chi = nbr_coefs(1) - forgot_chi;
            if (odd)
                --max_chi;
