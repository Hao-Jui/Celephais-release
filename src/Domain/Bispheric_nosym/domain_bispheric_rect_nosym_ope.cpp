...
Val_domain Domain_bispheric_rect_nosym::mult_cos_phi (const Val_domain& so) const {
	so.coef() ;
	Val_domain res(this) ;
	...
	do {
		switch ((*so.base.bases_1d[1])(index)) {
			...
			case LEG_EVEN :
				...
				break ;
				default :
					cout << "Unknown case in Domain_bispheric_rect_nosym::base_mult_sin_phi" << endl ;
					abort() ;
				}
		}
	while (index.inc()) ;
...
Val_domain Domain_bispheric_rect_nosym::mult_sin_phi (const Val_domain& so) const {
	so.coef() ;
	Val_domain res(this) ;
	...
	do {
		switch ((*so.base.bases_1d[1])(index)) {
			...
			case LEG_EVEN :
				...
				break ;
				default :
					cout << "Unknown case in Domain_bispheric_rect_nosym::base_mult_cos_phi" << endl ;
					abort() ;
				}
		}
	while (index.inc()) ;
...
Val_domain Domain_bispheric_rect_nosym::div_chi (const Val_domain& so) const {
	so.coef() ;
	Val_domain res(this) ;
...
Val_domain Domain_bispheric_rect_nosym::div_sin_chi(const Val_domain& so) const {
...
Val_domain Domain_bispheric_rect_nosym::mult_r (const Val_domain& so) const {
