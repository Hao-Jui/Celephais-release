...
namespace Kadath {
int Domain_bispheric_eta_first_nosym::nbr_conditions_val_domain_boundary (const Val_domain& so, int bound) const {
  ...
  if ((bound==ETA_MINUS_BC) || (bound==ETA_PLUS_BC)) {
	...
	for (int k=0 ; k<nbr_coefs(2) ; k++) {
			...
			int basechi = (*so.get_base().bases_1d[0]) (0,k) ;
			// No last coef if odd
			if (((basechi==CHEB_ODD) || (basechi==LEG_ODD)) && (i>=nbr_coefs(0)-2))
				true_other = false ;
			...
			if (((basechi==CHEB_EVEN) || (basechi==LEG_EVEN)) && (i>=nbr_coefs(0)-1))
				true_other = false ;
	...
	if (bound==OUTER_BC)
		res = (nbr_coefs(2)-2)*nbr_coefs(1) ;

	return res ;
}

Array<int> Domain_bispheric_eta_first_nosym::nbr_conditions_boundary (const Tensor& tt, int dom, int bound, int n_cmp, Array<int>** p_cmp) const {

	// Check boundary
	if ((bound!=ETA_MINUS_BC) && (bound!=ETA_PLUS_BC) && (bound!=OUTER_BC)) {
		cerr << "Unknown boundary in Domain_bispheric_eta_first_nosym::nbr_conditions_boundary" << endl ;
		abort() ;
	}
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_eta_first_nosym::nbr_conditions_boundary" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_eta_first_nosym::nbr_conditions_boundary" << endl ;
				abort() ;
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_bispheric_eta_first_nosym::nbr_conditions_boundary" << endl ;
			break ;
	}
	return res ;
}

bool Domain_bispheric_eta_first_nosym::describe_boundary_residual_rows(
    ...
    Array<int>** p_cmp, std::vector<ResidualRowDescriptor>& descriptors) const
{
    ...
    for (std::size_t slot = 0; slot < components.size(); ++slot) {
        ...
        for (int k = 0; k < nbr_coefs(2) - 1; ++k) {
            ...
            const int chi_basis = (*field.get_base().bases_1d[0])(0, k);
