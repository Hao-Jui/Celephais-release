...
namespace Kadath
{
    ...
    Val_domain Domain_shell_symphi::mult_cos_phi(const Val_domain& so) const
    {
        so.coef();
        Val_domain res(this);
    ...
    Val_domain Domain_shell_symphi::mult_sin_phi(const Val_domain& so) const
    {
        so.coef();
        Val_domain res(this);
    ...
    Val_domain Domain_shell_symphi::mult_cos_theta(const Val_domain& so) const
    {
        so.coef();
        Val_domain res(this);
        res.base = so.base;
    ...
    Val_domain Domain_shell_symphi::mult_sin_theta(const Val_domain& so) const
    {
    ...
    Val_domain Domain_shell_symphi::div_sin_theta(const Val_domain& so) const
    {
        so.coef();
        Val_domain res(this);
    ...
    Val_domain Domain_shell_symphi::div_cos_theta(const Val_domain& so) const
    {
        so.coef();
        Val_domain res(this);
    ...
    Val_domain Domain_shell_symphi::mult_r(const Val_domain& so) const
    {
        so.coef();
        Val_domain res(this);
    ...
    Val_domain Domain_shell_symphi::mult_1mrsL(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
    ...
    Val_domain Domain_shell_symphi::div_1mrsL(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        so.coef();
    ...
    Val_domain Domain_shell_symphi::div_r(const Val_domain& so) const
    {
    ...
    Val_domain Domain_shell_symphi::der_r(const Val_domain& so) const
    {
        return (so.der_var(1) / alpha);
    }

    Val_domain Domain_shell_symphi::der_partial_var(const Val_domain& so, int which_var) const
    {
        ...
        switch (which_var) {
            ...
            case 2:
                return so.der_var(3);
            default:
                KADATH_THROW("Unknown variable in Domain_shell_symphi::der_partial_var");
        }
    }

    Val_domain Domain_shell_symphi::ddp(const Val_domain& so) const
    {
        return (so.der_var(3).der_var(3));
    }

    Val_domain Domain_shell_symphi::div_xm1(const Val_domain& so) const
    {
        so.coef();
        Val_domain res(this);
    ...
    Val_domain Domain_shell_symphi::mult_xm1(const Val_domain& so) const
    {
        so.coef();
        Val_domain res(this);
    ...
    Val_domain Domain_shell_symphi::div_1mx2(const Val_domain& so) const
    {
        so.coef();
        Val_domain res(this);
    ...
    double Domain_shell_symphi::integ_volume(const Val_domain& so) const
    {
        ...
        if (so.check_if_zero())
            ...
            double val = 0;
            // Only k = 0
            assert((*integrant.get_base().bases_1d[1])(0) == SIN_ODD);
            Index pos(nbr_coefs);
            for (int j = 0; j < nbr_coefs(1); j++) {
                pos.set(1) = j;
                assert((*integrant.get_base().bases_1d[0])(j, 0) == CHEB);
