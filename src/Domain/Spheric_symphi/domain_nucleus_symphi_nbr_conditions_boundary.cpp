...
namespace Kadath
{
    ...
    Array<int> Domain_nucleus_symphi::nbr_conditions_boundary(const Tensor& tt, int dom, int bound, int n_cmp,
                                                              Array<int>** p_cmp) const
    {

        // Check boundary
        if (bound != OUTER_BC) {
            KADATH_THROW("Unknown boundary in Domain_nucleus_symphi::nbr_conditions_boundary");
        }
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_nucleus_symphi::nbr_conditions_boundary");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_nucleus_symphi::nbr_conditions_boundary");
                }
            } break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_nucleus_symphi::nbr_conditions_boundary"
                     << endl;
                break;
        }
        return res;
