...
namespace Kadath
{
    ...
    Array<int> Domain_critic_outer::nbr_conditions_boundary(const Tensor& tt, int dom, int bound, int n_cmp,
                                                            Array<int>**) const
    {

        // Check boundary
        if ((bound != INNER_BC) && (bound != OUTER_BC)) {
            KADATH_THROW("Unknown boundary in Domain_critic_outer::nbr_conditions_boundary");
        }
