...
namespace Kadath
{
    void Domain_critic_outer::find_other_dom(int dom, int bound, int& other_dom, int& other_bound) const
    {
        ...
        switch (bound) {
            case INNER_BC:
                ...
                break;
            default:
                KADATH_THROW("Unknown boundary case in Domain_critic_outer::find_other_dom");
        }
    }
    ...
    double Domain_critic_outer::val_boundary(int bound, const Val_domain& so, const Index& pos_cf) const
    {
        ...
        if (so.check_if_zero())
            ...
            switch (bound) {
                ...
                default:
                    KADATH_THROW("Unknown boundary type in Domain_critic_outer::val_boundary");
            }
            return res;
        }
    }
    ...
    int Domain_critic_outer::nbr_points_boundary(int bound, const Base_spectral& bb) const
    {
        ...
        if ((bound != INNER_BC) && (bound != OUTER_BC)) {
            KADATH_THROW("Unknown boundary in Domain_critic_outer::nbr_points_boundary");
        }

        int base_t = (*bb.bases_1d[1])(0);
        if ((base_t != COSSIN_EVEN) && (base_t != COSSIN_ODD)) {
            KADATH_THROW("Unknown base in Domain_critic_outer::nbr_conditions_val_domain_boundary");
        }
        return nbr_coefs(1) - 2;
    }

    void Domain_critic_outer::do_which_points_boundary(int bound, const Base_spectral&, Index** which_coef,
                                                       int start) const
    {
        ...
        switch (bound) {
            ...
            case OUTER_BC:
                ...
                break;
            default:
                KADATH_THROW("Unknown boundary in Domain_critic_outer::do_which_points_boundary");
        }
