...
namespace Kadath
{
    void Domain_critic_inner::export_tau_val_domain(const Val_domain& so, int order, Array<double>& sec, int& pos_sec,
                                                    int ncond) const
    {
        ...
        if (so.check_if_zero())
            ...
            if ((base_r == CHEB_EVEN) || (base_r == LEG_EVEN))
                maxi = nbr_coefs(0) - order;
            else {
                if ((base_r == CHEB_ODD) || (base_r == LEG_ODD))
                    maxi = nbr_coefs(0) - order - 1;
                else {
                    KADATH_THROW("Uknown base in Domain_critic_inner::export_tau");
                }
            }
    ...
    void Domain_critic_inner::export_tau(const Tensor& tt, int dom, int order, Array<double>& res, int& pos_res,
                                         const Array<int>& ncond, int, Array<int>**) const
    {
        ...
        switch (val) {
            case 0:
                ...
                break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_critic_inner::export_tau" << endl;
                break;
        }
    }
} // namespace Kadath
