...
namespace Kadath
{
    ...
    Term_eq Domain_polar_shell_outer_adapted::der_normal_term_eq(const Term_eq& so, int bound) const
    {
        ...
        switch (bound) {
            ...
            default:
                KADATH_THROW("Unknown boundary in Domain_polar_shell_outer_adapted::der_normal");
        }
    }
    ...
    Term_eq Domain_polar_shell_outer_adapted::lap_term_eq(const Term_eq& so, int mm) const
    {
        ...
        if (so.get_val_t().get_valence() != 0) {
            KADATH_THROW("Domain_polar_shell_outer_adapted::lap_term_eq only defined for scalars");
        }
    ...
    Term_eq Domain_polar_shell_outer_adapted::lap2_term_eq(const Term_eq& so, int mm) const
    {
        ...
        if (mm != 0) {
            KADATH_THROW("Domain_polar_shell_outer_adapted::lap2_term_eq not defined for m != 0 (for now)");
        }

        if (so.get_val_t().get_valence() != 0) {
            KADATH_THROW("Domain_polar_shell_outer_adapted::lap2_term_eq only defined for scalars");
        }
    ...
    const Term_eq* Domain_polar_shell_outer_adapted::give_normal(int bound, int tipe) const
    {
        ...
        switch (tipe) {
            ...
            default:
                KADATH_THROW("Unknown type of tensorial basis in Domain_polar_shell_outer_adapted::give_normal");
        }
    }

    Term_eq Domain_polar_shell_outer_adapted::integ_term_eq(const Term_eq&, int) const
    {
        KADATH_THROW("Domain_polar_shell_outer_adapted::integ_term_eq not implemented yet");
    }
} // namespace Kadath
