...
namespace Kadath
{
    ...
    Domain_polar_shell_bilateral_adapted::Domain_polar_shell_bilateral_adapted(const Space& sss, int num, int ttype,
                                                                               double rin, double rout, const Point& cr,
                                                                               const Dim_array& nbr)
        : Domain(num, ttype, nbr), sp(sss), center(cr)
    {
    ...
    // Constructor by copy
    Domain_polar_shell_bilateral_adapted::Domain_polar_shell_bilateral_adapted(
        const Domain_polar_shell_bilateral_adapted& so)
        : Domain(so), sp(so.sp), center(so.center)
    {
    ...
    // Constructor by copy
    Domain_polar_shell_bilateral_adapted::Domain_polar_shell_bilateral_adapted(
        const Space& sp, const Domain_polar_shell_bilateral_adapted& so)
        : Domain(so, true), sp(sp), center(so.center)
    {
    ...
    Domain_polar_shell_bilateral_adapted::Domain_polar_shell_bilateral_adapted(const Space& sss, int num,
                                                                               BinarySource& source)
        : Domain(num, source), sp(sss), center(source)
    {
        inner_radius = new Val_domain(this, source);
    ...
    void Domain_polar_shell_bilateral_adapted::do_radius() const
    {
        for (int i = 0; i < 2; i++)
        ...
        do
            radius->set(index) = ((*outer_radius)(index) - (*inner_radius)(index)) / 2. * ((*coloc[0])(index(0))) +
                                 ((*outer_radius)(index) + (*inner_radius)(index)) / 2.;
        while (index.inc());
        radius->std_r_base();
    }
    ...
    void Domain_polar_shell_bilateral_adapted::del_deriv() const
    {
        if (inner_radius_term_eq != nullptr)
            delete inner_radius_term_eq;
        inner_radius_term_eq = nullptr;
    ...
    int Domain_polar_shell_bilateral_adapted::nbr_unknowns_from_adapted() const
    {
        return 2 * nbr_coefs(1);
    }

    void Domain_polar_shell_bilateral_adapted::vars_to_terms() const
    {
    ...
    void Domain_polar_shell_bilateral_adapted::affecte_coef(int& conte, int cc, bool& found) const
    {

        Val_domain inner_auxi(this);
        inner_auxi.std_base();
        inner_auxi.set_in_coef();
        inner_auxi.allocate_coef();
        *inner_auxi.cf = 0;

        Val_domain outer_auxi(this);
        outer_auxi.std_base();
        outer_auxi.set_in_coef();
        outer_auxi.allocate_coef();
        *outer_auxi.cf = 0;
        ...
        if (found_inner) {
            Scalar auxi_scal(sp);
            auxi_scal.set_domain(num_dom) = inner_auxi;
        ...
        if (found_outer) {
            Scalar auxi_scal(sp);
            auxi_scal.set_domain(num_dom) = outer_auxi;
    ...
    void Domain_polar_shell_bilateral_adapted::xx_to_vars_from_adapted(Val_domain& new_outer_radius,
                                                                       const Array<double>& xx, int& pos) const
    {
    ...
    void Domain_polar_shell_bilateral_adapted::xx_to_vars_from_adapted(Val_domain& cor_inner, Val_domain& cor_outer,
                                                                       const Array<double>& xx, int& pos) const
    {
        cor_inner.allocate_coef();
        *cor_inner.cf = 0;
        cor_outer.allocate_coef();
        *cor_outer.cf = 0;
        ...
        for (int j = 0; j < nbr_coefs(1); j++) {
            pos_cf.set(1) = j;
            cor_inner.cf->set(pos_cf) -= xx(pos);
            pos++;
        }
        for (int j = 0; j < nbr_coefs(1); j++) {
            pos_cf.set(1) = j;
            cor_outer.cf->set(pos_cf) -= xx(pos);
            pos++;
        }

        cor_inner.set_base() = inner_radius->get_base();
        cor_outer.set_base() = outer_radius->get_base();
    }

    void Domain_polar_shell_bilateral_adapted::update_mapping(const Val_domain& cor)
    {
    ...
    void Domain_polar_shell_bilateral_adapted::update_mapping(const Val_domain& cor_inner, const Val_domain& cor_outer)
    {

        *inner_radius += cor_inner;
        *outer_radius += cor_outer;
        clear_import_plan_cache();
        for (int l = 0; l < ndim; l++) {
            if (absol[l] != nullptr)
                delete absol[l];
    ...
    void Domain_polar_shell_bilateral_adapted::set_mapping(const Val_domain& cor) const
    {
    ...
    void Domain_polar_shell_bilateral_adapted::update_variable(const Val_domain& cor_outer_radius, const Scalar& old,
                                                               Scalar& res) const
    {
    ...
    void Domain_polar_shell_bilateral_adapted::update_variable(const Val_domain& cor_inner_radius,
                                                               const Val_domain& cor_outer_radius, const Scalar& old,
                                                               Scalar& res) const
    {
        Val_domain dr(old(num_dom).der_r());
        if (dr.check_if_zero())
            ...
            res.set_domain(num_dom).allocate_conf();
            do {
                const double xi = (*coloc[0])(pos(0));
                res.set_domain(num_dom).set(pos) =
                    dr(pos) * ((1 - xi) * cor_inner_radius(pos) + (1 + xi) * cor_outer_radius(pos)) / 2.;
            } while (pos.inc());

            res.set_domain(num_dom) = res(num_dom) + old(num_dom);
            res.set_domain(num_dom).set_base() = old(num_dom).get_base();
        }
    }

    void Domain_polar_shell_bilateral_adapted::xx_to_ders_from_adapted(const Array<double>& xx, int& pos) const
    {
        ...
        *outer_auxi.cf = 0;
        ...
        for (int j = 0; j < nbr_coefs(1); j++) {
            pos_cf.set(1) = j;
            inner_auxi.cf->set(pos_cf) = xx(pos);
            pos++;
        }
        for (int j = 0; j < nbr_coefs(1); j++) {
            pos_cf.set(1) = j;
            outer_auxi.cf->set(pos_cf) = xx(pos);
            pos++;
        }
    ...
    void Domain_polar_shell_bilateral_adapted::update() const
    {
        ...
        Index index(nbr_points);
        do {
            val_res.set_domain(num_dom).set(index) = (((*outer_radius_term_eq->val_t)()(num_dom))(index) -
                                                      ((*inner_radius_term_eq->val_t)()(num_dom))(index)) /
                                                         2. * ((*coloc[0])(index(0))) +
                                                     (((*outer_radius_term_eq->val_t)()(num_dom))(index) +
                                                      ((*inner_radius_term_eq->val_t)()(num_dom))(index)) /
                                                         2.;
        } while (index.inc());
        val_res.set_domain(num_dom).std_r_base();

        bool doder = (outer_radius_term_eq->der_t != nullptr) || (inner_radius_term_eq->der_t != nullptr);
        if (doder) {
            Scalar der_res(sp);
            ...
            do {
                ...
                if (inner_radius_term_eq->der_t != nullptr)
                    inner = ((*inner_radius_term_eq->der_t)()(num_dom))(index);
                if (outer_radius_term_eq->der_t != nullptr)
                    outer = ((*outer_radius_term_eq->der_t)()(num_dom))(index);
                der_res.set_domain(num_dom).set(index) =
                    outer / 2. * (1 + (*coloc[0])(index(0))) + inner / 2. * (1 - (*coloc[0])(index(0)));
            } while (index.inc());
        ...
        val_res.set_domain(num_dom) =
            ((*outer_radius_term_eq->val_t)()(num_dom) - (*inner_radius_term_eq->val_t)()(num_dom)) / 2.;
        if (doder) {
            Scalar der_res(sp);
    ...
    void Domain_polar_shell_bilateral_adapted::save(BinarySink& sink) const
    {
        ...
        center.save(sink);
        inner_radius->save(sink);
        outer_radius->save(sink);
    }

    ostream& Domain_polar_shell_bilateral_adapted::print(ostream& o) const
    {
        o << "Adapted polar shell with bilateral boundaries" << endl;
    ...
    Val_domain Domain_polar_shell_bilateral_adapted::der_normal(const Val_domain&, int) const
    {
        KADATH_THROW("Domain_polar_shell_bilateral_adapted::der_normal not implemeted");
    }

    // Computes the cartesian coordinates
    void Domain_polar_shell_bilateral_adapted::do_absol() const
    {
        for (int i = 0; i < 2; i++)
            assert(coloc[i] != nullptr);
        for (int i = 0; i < 2; i++)
        ...
        do {
            ...
            double rr = ((*outer_radius)(index) - (*inner_radius)(index)) / 2. * ((*coloc[0])(index(0))) +
                        ((*outer_radius)(index) + (*inner_radius)(index)) / 2.;
            absol[0]->set(index) = rr * sin((*coloc[1])(index(1))) + center(1);
            absol[1]->set(index) = rr * cos((*coloc[1])(index(1))) + center(2);
        } while (index.inc());
    }

    // Computes the cartesian coordinates
    void Domain_polar_shell_bilateral_adapted::do_cart() const
    {
        for (int i = 0; i < 2; i++)
            assert(coloc[i] != nullptr);
        for (int i = 0; i < 2; i++)
        ...
        do {
            double rr = ((*outer_radius)(index) - (*inner_radius)(index)) / 2. * ((*coloc[0])(index(0))) +
                        ((*outer_radius)(index) + (*inner_radius)(index)) / 2.;
            cart[0]->set(index) = rr * sin((*coloc[1])(index(1))) + center(1);
            cart[1]->set(index) = rr * cos((*coloc[1])(index(1))) + center(2);
        } while (index.inc());
    }

    // Computes the cartesian coordinates over the radius
    void Domain_polar_shell_bilateral_adapted::do_cart_surr() const
    {
        for (int i = 0; i < 2; i++)
            assert(coloc[i] != nullptr);
        for (int i = 0; i < 2; i++)
    ...
    bool Domain_polar_shell_bilateral_adapted::is_in(const Point& xx, double prec) const
    {
    ...
    const Point Domain_polar_shell_bilateral_adapted::absol_to_num(const Point& abs) const
    {
        Point num(2);
        ...
        num.set(1) = (2. / (outer - inner)) * (air - (outer + inner) / 2.);

        return num;
    }

    double coloc_leg(int, int);
    void Domain_polar_shell_bilateral_adapted::do_coloc()
    {
        ...
        switch (type_base) {
            ...
            case LEG_TYPE:
                ...
                break;
            default:
                KADATH_THROW("Unknown type of basis in Domain_polar_shell_bilateral_adapted::do_coloc");
        }
    }

    void Domain_polar_shell_bilateral_adapted::set_cheb_base(Base_spectral& base) const
    {
        set_cheb_base_with_m(base, 0);
    }

    // standard base for a anti-symetric function in z, using Chebyshev
    void Domain_polar_shell_bilateral_adapted::set_anti_cheb_base(Base_spectral& base) const
    {
        set_anti_cheb_base_with_m(base, 0);
    }

    // standard base for a symetric function in z, using Legendre
    void Domain_polar_shell_bilateral_adapted::set_legendre_base(Base_spectral& base) const
    {
        set_legendre_base_with_m(base, 0);
    }

    // standard base for a anti-symetric function in z, using Legendre
    void Domain_polar_shell_bilateral_adapted::set_anti_legendre_base(Base_spectral& base) const
    {
        set_anti_legendre_base_with_m(base, 0);
    }

    // standard base for a symetric function in z, using Chebyshev
    void Domain_polar_shell_bilateral_adapted::set_cheb_base_with_m(Base_spectral& base, int m) const
    {
    ...
    // standard base for a anti-symetric function in z, using Chebyshev
    void Domain_polar_shell_bilateral_adapted::set_anti_cheb_base_with_m(Base_spectral& base, int m) const
    {
    ...
    // standard base for a symetric function in z, using Legendre
    void Domain_polar_shell_bilateral_adapted::set_legendre_base_with_m(Base_spectral& base, int m) const
    {
    ...
    // standard base for a anti-symetric function in z, using Legendre
    void Domain_polar_shell_bilateral_adapted::set_anti_legendre_base_with_m(Base_spectral& base, int m) const
    {
    ...
    void Domain_polar_shell_bilateral_adapted::set_cheb_r_base(Base_spectral& base) const
    {
        set_cheb_base_with_m(base, 0);
    }

    void Domain_polar_shell_bilateral_adapted::set_legendre_r_base(Base_spectral& base) const
    {
        set_legendre_base_with_m(base, 0);
    }

    // Computes the derivatives with respect to XYZ as a function of the numerical ones.
    void Domain_polar_shell_bilateral_adapted::do_der_abs_from_der_var(Val_domain** der_var, Val_domain** der_abs) const
    {
    ...
    Base_spectral Domain_polar_shell_bilateral_adapted::mult(const Base_spectral& a, const Base_spectral& b) const
    {
    ...
    void Domain_polar_shell_bilateral_adapted::update_constante(const Val_domain& cor_outer_radius, const Scalar& old,
                                                                Scalar& res) const
    {
        ...
        do {
            ...
            double rr =
                ((*outer_radius)(pos) + cor_outer_radius(pos) - (*inner_radius)(pos)) * (*coloc[0])(pos(0)) / 2. +
                ((*outer_radius)(pos) + cor_outer_radius(pos) + (*inner_radius)(pos)) / 2.;
    ...
    void Domain_polar_shell_bilateral_adapted::update_constante(const Val_domain& cor_inner_radius,
                                                                const Val_domain& cor_outer_radius, const Scalar& old,
                                                                Scalar& res) const
    {
        Point MM(2);
        ...
        do {
            double rr =
                ((*outer_radius)(pos) + cor_outer_radius(pos) - (*inner_radius)(pos)-cor_inner_radius(pos)) *
                    (*coloc[0])(pos(0)) / 2. +
                ((*outer_radius)(pos) + cor_outer_radius(pos) + (*inner_radius)(pos) + cor_inner_radius(pos)) / 2.;
            double theta = (*coloc[1])(pos(1));
    ...
    int Domain_polar_shell_bilateral_adapted::give_place_var(char* p) const
    {
        int res = -1;
        if (strcmp(p, "R ") == 0)
            res = 0;
    ...
    Val_domain Domain_polar_shell_bilateral_adapted::dt(const Val_domain& so) const
    {
        return (so.der_var(2));
    }
} // namespace Kadath
