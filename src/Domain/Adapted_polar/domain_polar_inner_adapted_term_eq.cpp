...
namespace Kadath
{
    ...
    Term_eq Domain_polar_shell_inner_adapted::derive_r(const Term_eq& so) const
    {
    ...
    Term_eq Domain_polar_shell_inner_adapted::derive_t(const Term_eq& so) const
    {
    ...
    Term_eq Domain_polar_shell_inner_adapted::flat_grad_spher(const Term_eq& so) const
    {
    ...
    void Domain_polar_shell_inner_adapted::do_normal_spher() const
    {
    ...
    void Domain_polar_shell_inner_adapted::do_normal_cart() const
    {
    ...
    Term_eq Domain_polar_shell_inner_adapted::der_normal_term_eq(const Term_eq& so, int bound) const
    {
        ...
        switch (bound) {
            ...
            default:
                KADATH_THROW("Unknown boundary in Domain_polar_shell_inner_adapted::der_normal");
        }
    }

    Term_eq Domain_polar_shell_inner_adapted::lap_term_eq(const Term_eq& so, int mm) const
    {

        if (so.get_val_t().get_valence() != 0) {
            KADATH_THROW("Domain_polar_shell_inner_adapted::lap_term_eq only defined for scalars");
        }
    ...
    Term_eq Domain_polar_shell_inner_adapted::lap2_term_eq(const Term_eq& so, int mm) const
    {

        if (mm != 0) {
            KADATH_THROW("Domain_polar_shell_inner_adapted::lap2_term_eq not defined for m != 0 (for now)");
        }

        if (so.get_val_t().get_valence() != 0) {
            KADATH_THROW("Domain_polar_shell_inner_adapted::lap2_term_eq only defined for scalars");
        }
    ...
    Term_eq Domain_polar_shell_inner_adapted::mult_r_term_eq(const Term_eq& so) const
    {

        return so * (*rad_term_eq);
    }

    Term_eq Domain_polar_shell_inner_adapted::div_r_term_eq(const Term_eq& so) const
    {

        return so / (*rad_term_eq);
    }

    void Domain_polar_shell_inner_adapted::update_term_eq(Term_eq* so) const
    {
        ...
        for (int cmp = 0; cmp < so->val_t->get_n_comp(); cmp++) {
            ...
            if (!derr.check_if_zero()) {
                ...
                do {
                    res.set(pos) = (*(*inner_radius_term_eq->der_t).cmp[0])(num_dom)(pos) / 2. *
    ...
    Term_eq Domain_polar_shell_inner_adapted::partial_spher(const Term_eq& so) const
    {
        int dom = so.get_dom();
        assert(dom == num_dom);
    ...
    Term_eq Domain_polar_shell_inner_adapted::partial_cart(const Term_eq& so) const
    {
        int dom = so.get_dom();
        assert(dom == num_dom);
    ...
    Term_eq Domain_polar_shell_inner_adapted::grad_term_eq(const Term_eq& so) const
    {
        return partial_cart(so);
    }

    const Term_eq* Domain_polar_shell_inner_adapted::give_normal(int bound, int tipe) const
    {
        ...
        switch (tipe) {
            ...
            default:
                KADATH_THROW("Unknown type of tensorial basis in Domain_polar_shell_inner_adapted::give_normal");
        }
    }

    Term_eq Domain_polar_shell_inner_adapted::integ_term_eq(const Term_eq&, int) const
    {
        KADATH_THROW("Domain_polar_shell_inner_adapted::integ_term_eq not implemented yet");
    }
} // namespace Kadath
