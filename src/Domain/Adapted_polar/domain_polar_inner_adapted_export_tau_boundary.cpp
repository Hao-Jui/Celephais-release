...
namespace Kadath
{
    void Domain_polar_shell_inner_adapted::export_tau_val_domain_boundary(const Val_domain& so, int mquant, int bound,
            ...
            for (int j = 0; j < nbr_coefs(1); j++) {
                ...
                switch (baset) {
                    ...
                    case SIN_ODD:
                        ...
                        break;
                    default:
                        KADATH_THROW("Unknow theta basis in Domain_polar_shell_inner_adapted::export_tau_val_domain_boundary");
                }
            }
        }
    }

    void Domain_polar_shell_inner_adapted::export_tau_boundary(const Tensor& tt, int dom, int bound, Array<double>& res,
                                                               ...
                                                               Array<int>**) const
    {
        ...
        if ((bound != INNER_BC) && (bound != OUTER_BC)) {
            KADATH_THROW("Unknown boundary in Domain_polar_shell_inner_adapted::export_tau_boundary");
        }
        ...
        switch (val) {
            case 0:
                ...
                break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_polar_shell_inner_adapted::export_tau_boundary"
                     << endl;
                break;
        }
    }
