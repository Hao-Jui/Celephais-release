...
namespace Kadath
{
        ...
        for (int j = 0; j < nbr_coefs(1); j++) {
            ...
            switch (baset) {
                ...
                case SIN_ODD:
                    ...
                    break;
                default:
                    KADATH_THROW("Unknow theta basis in Domain_polar_shell_outer_adapted::nbr_conditions_val_boundary");
            }
    ...
    Array<int> Domain_polar_shell_outer_adapted::nbr_conditions_boundary(const Tensor& tt, int dom, int bound,
                                                                         int n_cmp, Array<int>**) const
    {

        // Check boundary
        if ((bound != INNER_BC) && (bound != OUTER_BC)) {
            KADATH_THROW("Unknown boundary in Domain_polar_shell_outer_adapted::nbr_conditions_boundary");
        }
        ...
        switch (val) {
            ...
            default:
                cerr << "Valence " << val
                     << " not implemented in Domain_polar_shell_outer_adapted::nbr_conditions_boundary" << endl;
                break;
        }
        return res;
    }
