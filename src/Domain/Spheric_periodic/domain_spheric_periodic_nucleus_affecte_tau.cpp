...
namespace Kadath
{
    void Domain_spheric_periodic_nucleus::affecte_tau_val_domain(Val_domain& so, const Array<double>& values,
                                                                 int& conte) const
    {
        ...
        for (int j = 0; j < nbr_coefs(1); j++) {
            ...
            switch (baset) {
                ...
                case COS:
                    break;
                default:
                    KADATH_THROW("Unknow time basis in Domain_spheric_periodic_nucleus::affecte_tau_val_domain");
            }
            if (true_time)
    ...
    void Domain_spheric_periodic_nucleus::affecte_tau(Tensor& tt, int dom, const Array<double>& cf, int& pos_cf) const
    {
