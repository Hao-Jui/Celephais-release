...
namespace Kadath
{
    ...
    void Domain_shell_inner_adapted::do_normal_spher() const
    {
        ...
        bool doder = ((grad.der_t == nullptr) || (inner_radius_term_eq->der_t == nullptr)) ? false : true;
        if (doder) {
            Scalar der_norme(sp);
    ...
    Term_eq Domain_shell_inner_adapted::der_normal_term_eq(const Term_eq& so, int bound) const
    {
        ...
        switch (bound) {
            ...
            default:
                KADATH_THROW("Unknown boundary in Domain_shell_inner_adapted::der_normal");
        }
    }
    ...
    {
        if (mm != 0) {
            KADATH_THROW("Domain_shell_inner_adapted::lap_term_eq not defined for m != 0 (for now)");
        }

        if (so.get_val_t().get_valence() != 0) {
            KADATH_THROW("Domain_shell_inner_adapted::lap_term_eq only defined for scalars");
        }
                    ...
                    case 1:
                        ...
                        switch (pos_auxi(ind_sum + 1)) {
                            ...
                            case 2:
                                ...
                                break;
                            default:
                                KADATH_THROW("Bad indice in Domain_shell_inner_adapted::connection_spher");
                        }
                        break;
                    case 2:
                        // Dphi
