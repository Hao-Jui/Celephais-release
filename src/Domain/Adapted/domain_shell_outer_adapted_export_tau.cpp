...
namespace Kadath
{
    void Domain_shell_outer_adapted::export_tau_val_domain(const Val_domain& so, int mlim, int order,
                            ...
                            switch (baset) {
                                case COS_EVEN:
                                    if (k < kmin) {
                                        ...
                                        sec.set(pos_sec) = (*so.cf)(pos_cf)-2 * (*so.cf)(pos_galerkin);
                                        pos_sec++;
                                    }
                                    break;
                                case COS_ODD:
                                    if (j != nbr_coefs(1) - 1) {
                                        if (k < kmin) {
                                            ...
                                            pos_galerkin = pos_cf;
                                            pos_galerkin.set(1) = 0;
                                            sec.set(pos_sec) = (*so.cf)(pos_cf) - (*so.cf)(pos_galerkin);
                                            pos_sec++;
                                        }
                                    }
                                    break;
                                case SIN_EVEN:
                                    if ((j != 0) && (j != nbr_coefs(1) - 1)) {
                                        if (k < kmin + 2) {
                                            ...
                                            sec.set(pos_sec) = (*so.cf)(pos_cf)-j * (*so.cf)(pos_galerkin);
                                            pos_sec++;
                                        }
                                    }
                                    break;
                                case SIN_ODD:
                                    if (j != nbr_coefs(1) - 1) {
                                        if (k < kmin + 2) {
                                            ...
                                            pos_galerkin = pos_cf;
                                            pos_galerkin.set(1) = 0;
                                            sec.set(pos_sec) = (*so.cf)(pos_cf) - (2 * j + 1) * (*so.cf)(pos_galerkin);
                                            pos_sec++;
                                        }
                                    }
                                    break;
                                default:
                                    KADATH_THROW("Unknow theta basis in Domain_shell_outer_adapted::export_tau_val_domain");
                            }
                        }
                    }
                }
    ...
    void Domain_shell_outer_adapted::export_tau(const Tensor& tt, int dom, int order, Array<double>& res, int& pos_res,
                                                const Array<int>& ncond, int n_cmp, Array<int>** p_cmp) const
    {
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_shell_outer_adapted::export_tau");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_shell_outer_adapted::export_tau");
                }
            } break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_shell_outer_adapted::export_tau" << endl;
                break;
        }
    }
} // namespace Kadath
