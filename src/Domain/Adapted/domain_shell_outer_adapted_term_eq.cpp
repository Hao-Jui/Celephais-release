...
namespace Kadath
{
    ...
    void Domain_shell_outer_adapted::do_normal_spher() const
    {
        ...
        bool doder = ((grad.der_t == nullptr) || (outer_radius_term_eq->der_t == nullptr)) ? false : true;
        if (doder) {
            Scalar der_norme(sp);
            der_norme.set_domain(num_dom) = ((*grad.der_t)(1)(num_dom) * (*grad.val_t)(1)(num_dom) +
                                             (*grad.der_t)(2)(num_dom) * (*grad.val_t)(2)(num_dom) +
                                             (*grad.der_t)(3)(num_dom) * (*grad.val_t)(3)(num_dom)) /
                                            val_norme(num_dom);
    ...
    void Domain_shell_outer_adapted::do_normal_cart() const
    {
        ...
        bool doder = (normal_spher->der_t == nullptr) ? false : true;
        if (doder) {
            Vector der(sp, CON, basis);

            der.set(1).set_domain(num_dom) = mult_cos_phi(mult_sin_theta((*normal_spher->der_t)(1)(num_dom)) +
                                                          mult_cos_theta((*normal_spher->der_t)(2)(num_dom))) -
                                             mult_sin_phi((*normal_spher->der_t)(3)(num_dom));
            der.set(2).set_domain(num_dom) = mult_sin_phi(mult_sin_theta((*normal_spher->der_t)(1)(num_dom)) +
                                                          mult_cos_theta((*normal_spher->der_t)(2)(num_dom))) +
                                             mult_cos_phi((*normal_spher->der_t)(3)(num_dom));
            der.set(3).set_domain(num_dom) =
                mult_cos_theta((*normal_spher->der_t)(1)(num_dom)) - mult_sin_theta((*normal_spher->der_t)(2)(num_dom));
        ...
        switch (bound) {
            case OUTER_BC: {
                ...
                auto build_normal_derivative = [&](int lane) {
                    Tensor derivative(so.get_val_t(), false);
            ...
            default:
                KADATH_THROW("Unknown boundary in Domain_shell_outer_adapted::der_normal");
        }
    }
    ...
    {
        if (mm != 0) {
            KADATH_THROW("Domain_shell_outer_adapted::lap_term_eq not defined for m != 0 (for now)");
        }

        if (so.get_val_t().get_valence() != 0) {
            KADATH_THROW("Domain_shell_outer_adapted::lap_term_eq only defined for scalars");
        }
    ...
    void Domain_shell_outer_adapted::update_term_eq(Term_eq* so) const
    {
        ...
        for (int cmp = 0; cmp < so->val_t->get_n_comp(); cmp++) {
            ...
            if (!derr.check_if_zero()) {
                ...
                do {
                    res.set(pos) = (*(*outer_radius_term_eq->der_t).cmp[0])(num_dom)(pos) / 2. *
                    ...
                    case 1:
                        ...
                        switch (pos_auxi(ind_sum + 1)) {
                            ...
                            case 2:
                                ...
                                break;
                            default:
                                KADATH_THROW("Bad indice in Domain_shell_outer_adapted::connection_spher");
                        }
                        break;
                    case 2:
                        // Dphi
    ...
    const Term_eq* Domain_shell_outer_adapted::give_normal(int bound, int tipe) const
    {
        ...
        switch (tipe) {
            ...
            default:
                KADATH_THROW("Unknown type of tensorial basis in Domain_shell_inner_adapted::give_normal");
        }
    }
    ...
    Term_eq Domain_shell_outer_adapted::integ_volume_term_eq(const Term_eq& target) const
    {
        ...
        auto integrate_adapted_volume = [&](const Val_domain& value) {
            ...
            for (int j = 0; j < nbr_coefs(1); j++) {
                ...
                integral += 2. / (2. * j + 1) * integral_1d(CHEB, cf);
            }
    ...
    {
        KADATH_THROW("Domain_shell_outer_adapted::integ_term_eq not implemented yet");
    }
} // namespace Kadath
