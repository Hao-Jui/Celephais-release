...
namespace Kadath
{
    ...
    Domain_shell_outer_adapted::Domain_shell_outer_adapted(const Space& sss, int num, int ttype, double rin,
    ...
    // Constructor by copy
    Domain_shell_outer_adapted::Domain_shell_outer_adapted(const Domain_shell_outer_adapted& so)
        : Domain(so), sp(so.sp), inner_radius(so.inner_radius), center(so.center)
    {
    ...
    Domain_shell_outer_adapted::Domain_shell_outer_adapted(const Space& sss, int num, BinarySource& source)
    ...
    void Domain_shell_outer_adapted::do_radius() const
    {
        ...
        do
            radius->set(index) = ((*outer_radius)(index)-inner_radius) / 2. * ((*coloc[0])(index(0))) +
    ...
    void Domain_shell_outer_adapted::xx_to_vars_from_adapted(Val_domain& new_outer_radius, const Array<double>& xx,
                                                             int& pos) const
    {
    ...
    void Domain_shell_outer_adapted::update_variable(const Val_domain& cor_outer_radius, const Scalar& old,
                                                     Scalar& res) const
    {
    ...
    void Domain_shell_outer_adapted::update() const
    {
        ...
        bool doder = (outer_radius_term_eq->der_t == nullptr) ? false : true;
        if (doder) {
            Scalar der_res(sp);
            if ((*outer_radius_term_eq->der_t)()(num_dom).check_if_zero()) {
                ...
                do {
                    der_res.set_domain(num_dom).set(index) =
                        ((*outer_radius_term_eq->der_t)()(num_dom))(index) / 2. * ((*coloc[0])(index(0))) +
    ...
    void Domain_shell_outer_adapted::save(BinarySink& sink) const
    {
    ...
    ostream& Domain_shell_outer_adapted::print(ostream& o) const
    {
    ...
    {
        KADATH_THROW("Domain_shell_outer_adapted::der_normal not implemeted");
    }
    ...
    const Point Domain_shell_outer_adapted::absol_to_num_bound(const Point& abs, int bound) const
    {
    ...
    void Domain_shell_outer_adapted::do_coloc()
    {
        ...
        switch (type_base) {
            ...
            case LEG_TYPE:
                ...
                break;
            default:
                KADATH_THROW("Unknown type of basis in Domain_shell_outer_adapted::do_coloc");
        }
    }
    ...
    void Domain_shell_outer_adapted::update_constante(const Val_domain& cor_outer_radius, const Scalar& old,
                                                      Scalar& res) const
    {
