...
namespace Kadath
{
    ...
    Val_domain Domain_shell_outer_adapted::mult_cos_phi(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
            ...
            do {
                switch ((*so.base.bases_1d[1])(index_t)) {
                    ...
                    case SIN_ODD:
                        ...
                        break;
                    default:
                        KADATH_THROW("Unknown case in Domain_shell_outer_adapted::mult_cos_phi");
                }
            } while (index_t.inc());
    ...
    Val_domain Domain_shell_outer_adapted::mult_sin_phi(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
            ...
            do {
                switch ((*so.base.bases_1d[1])(index_t)) {
                    ...
                    case SIN_ODD:
                        ...
                        break;
                    default:
                        KADATH_THROW("Unknown case in Domain_shell_outer_adapted::mult_sin_phi");
                }
            } while (index_t.inc());
            *res.base.bases_1d[0] = *so.base.bases_1d[0];
    ...
    Val_domain Domain_shell_outer_adapted::mult_cos_theta(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
    ...
    Val_domain Domain_shell_outer_adapted::mult_sin_theta(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
    ...
    Val_domain Domain_shell_outer_adapted::div_sin_theta(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
    ...
    Val_domain Domain_shell_outer_adapted::div_cos_theta(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
    ...
    Val_domain Domain_shell_outer_adapted::ddp(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
    ...
    Val_domain Domain_shell_outer_adapted::der_r(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
    ...
    Val_domain Domain_shell_outer_adapted::div_r(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
