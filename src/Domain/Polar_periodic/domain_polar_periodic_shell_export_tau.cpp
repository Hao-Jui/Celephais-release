...
namespace Kadath
{
    void Domain_polar_periodic_shell::export_tau_val_domain(const Val_domain& so, int order, Array<double>& sec,
        ...
        if (so.check_if_zero())
            ...
            switch (basetime) {
                ...
                case SIN:
                    ...
                    break;
                default:
                    KADATH_THROW("Unknown time basis in Domain_polar_periodic_shell_export_tau_val_domain");
            }
    ...
    void Domain_polar_periodic_shell::export_tau(const Tensor& tt, int dom, int order, Array<double>& res, int& pos_res,
