...
namespace Kadath
{
    int Domain_polar_compact::nbr_unknowns_val_domain(const Val_domain& so, int mquant) const
    {
        ...
        do {
            ...
            switch (baset) {
                ...
                case SIN_ODD:
                    ...
                    break;
                default:
                    KADATH_THROW("Unknow theta basis in Domain_polar_compact::nbr_unknowns_val_domain");
            }
            if (indic)
                res++;
        } while (pos.inc());
    ...
    int Domain_polar_compact::nbr_unknowns(const Tensor& tt, int dom) const
    {
