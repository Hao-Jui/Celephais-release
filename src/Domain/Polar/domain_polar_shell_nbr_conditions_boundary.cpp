...
namespace Kadath
{
    int Domain_polar_shell::nbr_conditions_val_domain_boundary(const Val_domain& so, int mquant) const
    {
        ...
        for (int j = 0; j < nbr_coefs(1); j++) {
            ...
            switch (baset) {
                ...
                case SIN_ODD:
                    ...
                    break;
                default:
                    KADATH_THROW("Unknow theta basis in Domain_polar_shell::nbr_conditions_val_boundary");
            }
    ...
    Array<int> Domain_polar_shell::nbr_conditions_boundary(const Tensor& tt, int dom, int bound, int n_cmp,
                                                           Array<int>**) const
    {

        // Check boundary
        if ((bound != INNER_BC) && (bound != OUTER_BC)) {
            KADATH_THROW("Unknown boundary in Domain_polar_shell::nbr_conditions_boundary");
        }
        ...
        switch (val) {
            case 0:
                ...
                break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_polar_shell::nbr_conditions_boundary" << endl;
                break;
        }
        return res;
    }
