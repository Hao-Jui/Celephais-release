...
namespace Kadath
{
    ...
    Array<int> Domain_oned_inf::nbr_conditions_boundary(const Tensor& tt, int dom, int bound, int n_cmp,
                                                        Array<int>**) const
    {

        // Check boundary
        if ((bound != OUTER_BC) && (bound != INNER_BC)) {
            KADATH_THROW("Unknown boundary in Domain_oned_inf::nbr_conditions_boundary");
        }
