...
namespace Kadath
{
    void Domain_oned_inf::export_tau_val_domain_boundary(const Val_domain& so, int bound, Array<double>& sec,
    ...
    void Domain_oned_inf::export_tau_boundary(const Tensor& tt, int dom, int bound, Array<double>& res, int& pos_res,
                                              const Array<int>& ncond, int, Array<int>**) const
    {

        // Check boundary
        if ((bound != OUTER_BC) && (bound != INNER_BC)) {
            KADATH_THROW("Unknown boundary in Domain_oned_inf::export_tau_boundary");
        }
