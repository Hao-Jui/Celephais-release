    ...
    void Domain_oned_ori::find_other_dom(int dom, int bound, int& other_dom, int& other_bound) const
    {
        ...
        switch (bound) {
            case OUTER_BC:
                ...
                break;
            default:
                KADATH_THROW("Unknown boundary case in Domain_oned_ori::find_other_dom");
        }
    }

    double Domain_oned_ori::val_boundary(int bound, const Val_domain& so, const Index& pos_cf) const
    {
        ...
        if (so.check_if_zero())
            ...
            switch (bound) {
                case OUTER_BC:
                    ...
                    break;
                default:
                    KADATH_THROW("Unknown boundary type in Domain_oned_ori::val_boundary");
            }
            return res;
        }
    }
