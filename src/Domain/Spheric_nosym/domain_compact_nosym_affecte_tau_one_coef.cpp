...
void Domain_compact_nosym::affecte_tau_one_coef_val_domain (Val_domain& so, int mlim, int cc, int& conte) const {
...
void Domain_compact_nosym::affecte_tau_one_coef (Tensor& tt, int dom, int cc, int& pos_cf) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				KADATH_THROW("Unknown type of vector Domain_compact_nosym::affecte_tau_one_coef");
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				KADATH_THROW("Unknown type of 2-tensor Domain_compact_nosym::affecte_tau_one_coef");
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_compact_nosym::affecte_tau" << endl ;
			break ;
	}
}

bool Domain_compact_nosym::describe_tau_seed_block(
    const Tensor& tt, int dom, std::vector<TauSeedDescriptor>& descriptors) const
{
    ...
    for (int component = 0; component < tt.get_n_comp(); ++component) {
        ...
        detail::append_spheric_shell_tau_seed_component(
            nbr_coefs, tt(index)(dom), component, 0, descriptors);
    }
