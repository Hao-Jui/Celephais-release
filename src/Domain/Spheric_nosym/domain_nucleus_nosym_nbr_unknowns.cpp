...
int Domain_nucleus_nosym::nbr_unknowns_val_domain (const Val_domain& so) const {
		...
		if (indic) {
			...
			switch (baser) {
				...
				case LEG_EVEN :
					...
					break ;
				case CHEB_ODD :
					if (((pos(2)>=4) && (pos(0)==0)) || ((pos(1)>1) && (pos(0)==0)) || (pos(0)==nbr_coefs(0)-1))
						indic = false ;
					break ;
				case LEG_ODD :
					if (((pos(2)>=4) && (pos(0)==0)) || ((pos(1)>1) && (pos(0)==0)) || (pos(0)==nbr_coefs(0)-1))
					...
					break ;
				default:
					KADATH_THROW("Unknown radial basis in Domain_nucleus::nbr_unknowns_val_domain");
			}
		}
...
int Domain_nucleus_nosym::nbr_unknowns (const Tensor& tt, int dom) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				KADATH_THROW("Unknown type of vector Domain_nucleus_nosym::nbr_unknowns");
			}
		}
		    break ;
		case 2 : {
			...
			if (!found) {
				KADATH_THROW("Unknown type of 2-tensor Domain_nucleus_nosym::nbr_unknowns");
			}
		}
		break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_nucleus_nosym::nbr_unknowns" << endl ;
			break ;
	}
	return res ;
}}
