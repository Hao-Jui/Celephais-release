...
void Domain_compact_nosym::export_tau_val_domain_boundary (const Val_domain& so, int mlim, int bound, Array<double>& sec, int& pos_sec, int ncond) const {
	...
	for (int k=0 ; k<nbr_coefs(2)-1 ; k++)
		if (k!=1) {
			...
			const bool uses_theta_galerkin = detail::spheric_nosym_uses_theta_galerkin(baset, k, kmin) ;
			...
			for (int j=0 ; j<nbr_coefs(1) ; j++) {
				...
				switch (baset) {
					...
					case SIN:
						...
						break ;
					default:
						KADATH_THROW("Unknow theta basis in Domain_compact_nosym::export_tau_val_domain_boundary");
					}
				}
		}
	}
...
void Domain_compact_nosym::export_tau_boundary (const Tensor& tt, int dom, int bound, Array<double>& res, int& pos_res, const Array<int>& ncond,
										int n_cmp, Array<int>** p_cmp) const {

	// Check boundary
	if ((bound!=INNER_BC) && (bound!=OUTER_BC)) {
		KADATH_THROW("Unknown boundary in Domain_compact_nosym::export_tau_boundary");
	}
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				KADATH_THROW("Unknown type of vector Domain_compact_nosym::export_tau_boundary");
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				KADATH_THROW("Unknown type of 2-tensor Domain_compact_nosym::export_tau_boundary");
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_compact_nosym::export_tau_boundary" << endl ;
			break ;
	}
}}
