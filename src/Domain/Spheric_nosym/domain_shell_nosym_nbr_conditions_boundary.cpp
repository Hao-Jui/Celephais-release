...
int Domain_shell_nosym::nbr_conditions_val_domain_boundary (const Val_domain& so, int mlim) const {
...
Array<int> Domain_shell_nosym::nbr_conditions_boundary (const Tensor& tt, int dom, int bound, int n_cmp, Array<int>** p_cmp) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				KADATH_THROW("Unknown type of vector Domain_shell_nosym::nbr_conditions_boundary");
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				KADATH_THROW("Unknown type of 2-tensor Domain_shell_nosym::nbr_conditions_boundary");
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_shell_nosym::nbr_conditions_boundary" << endl ;
			break ;
	}
	return res ;
}

bool Domain_shell_nosym::describe_boundary_residual_rows(
