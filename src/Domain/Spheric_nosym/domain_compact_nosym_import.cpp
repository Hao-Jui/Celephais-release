...
namespace Kadath {
// Tensorial parts :
Tensor Domain_compact_nosym::import (int numdom, int bound, int n_ope, const Array<int>& zedoms, Tensor** parts) const {
  ...
  for (int k=0 ; k<nbr_points(2) ; k++) {
	...
	if (parts[0]->get_space().get_domain(zedoms(current))->is_in(MM))
      ...
      if (!found) {
	cerr << "Point " << MM << " not found in other domains, for Domain_compact_nosym::import" << endl ;
