...
// A.5-validated inline boundary emit (74e7f645). HEAD `mlim` retained for
// signature/API compatibility but unused (reference logic threads no m-order).
void Domain_nucleus_nosym::export_tau_val_domain_boundary (const Val_domain& so, int mlim, int bound, Array<double>& sec, int& pos_sec, int ncond) const {
	(void)mlim;
	if (so.check_if_zero())
		pos_sec += ncond ;
	else {
	...
	for (int k=0 ; k<nbr_coefs(2)-1 ; k++)
		if (k!=1) {
			...
			const bool uses_theta_galerkin = detail::spheric_nosym_uses_theta_galerkin(baset, k, 2) ;
			...
			for (int j=0 ; j<nbr_coefs(1) ; j++) {
				...
				switch (baset) {
					...
					default:
						KADATH_THROW("Unknow theta basis in Domain_nucleus_nosym::export_tau_val_domain_boundary");
					}
				}
		}
	}
...
void Domain_nucleus_nosym::export_tau_boundary (const Tensor& tt, int dom, int bound, Array<double>& res, int& pos_res, const Array<int>& ncond,
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				KADATH_THROW("Unknown type of vector Domain_nucleus_nosym::export_tau_boundary");
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				KADATH_THROW("Unknown type of 2-tensor Domain_nucleus_nosym::export_tau_boundary");
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_nucleus_nosym::export_tau_boundary" << endl ;
			break ;
	}
}}
