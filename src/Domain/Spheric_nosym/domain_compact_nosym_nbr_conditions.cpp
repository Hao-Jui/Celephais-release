...
int Domain_compact_nosym::nbr_conditions_val_domain (const Val_domain& so, int mlim, int order) const {
...
Array<int> Domain_compact_nosym::nbr_conditions (const Tensor& tt, int dom, int order, int n_cmp, Array<int>** p_cmp) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				KADATH_THROW("Unknown type of vector Domain_compact_nosym::nbr_conditions");
			}
		}
		      break ;
		case 2 : {
			...
			if (!found) {
				KADATH_THROW("Unknown type of 2-tensor Domain_compact_nosym::nbr_conditions");
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_compact_nosym::nbr_conditions" << endl ;
			break ;
	}
	return res ;
}

bool Domain_compact_nosym::describe_volume_residual_rows(
