...
void Domain_shell_nosym::export_tau_val_domain (const Val_domain& so, int mlim, int order, Array<double>& sec, int& pos_sec, int ncond) const {
			...
			for (int j=0 ; j<nbr_coefs(1) ; j++) {
				...
				for (int i=0 ; i<nbr_coefs(0)-order ; i++) {
					...
					case SIN:
						...
						break ;
					default:
						KADATH_THROW("Unknow theta basis in Domain_shell_nosym::export_tau_val_domain");
					}
				}
			}
		}
...
void Domain_shell_nosym::export_tau (const Tensor& tt, int dom, int order, Array<double>& res, int& pos_res, const Array<int>& ncond,
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				KADATH_THROW("Unknown type of vector Domain_shell_nosym::export_tau");
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				KADATH_THROW("Unknown type of 2-tensor Domain_shell_nosym::export_tau");
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_shell_nosym::export_tau" << endl ;
			break ;
	}
}}
