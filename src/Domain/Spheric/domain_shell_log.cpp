...
namespace Kadath
{
    ...
    Domain_shell_log::Domain_shell_log(int num, int ttype, double rint, double rext, const Point& cr,
                                       const Dim_array& nbr)
    ...
    Domain_shell_log::Domain_shell_log(int num, BinarySource& source) : Domain_shell(num, source)
    {
        do_coloc();
    }
    ...
    void Domain_shell_log::save(BinarySink& sink) const
    {
    ...
    Val_domain Domain_shell_log::der_normal(const Val_domain& so, int bound) const
    {
        ...
        if ((bound != OUTER_BC) && (bound != INNER_BC)) {
            KADATH_THROW("Unknown boundary case in Domain_shell_log::der_normal");
        }
        return der_r(so);
    }
    ...
    void Domain_shell_log::do_absol() const
    {
        ...
        do {
            absol[0]->set(index) =
                exp(alpha * ((*coloc[0])(index(0))) + beta) * sin((*coloc[1])(index(1))) * cos((*coloc[2])(index(2))) +
                center(1);
            absol[1]->set(index) =
                exp(alpha * ((*coloc[0])(index(0))) + beta) * sin((*coloc[1])(index(1))) * sin((*coloc[2])(index(2))) +
                center(2);
    ...
    bool Domain_shell_log::is_in(const Point& xx, double prec) const
    {
        ...
        bool res =
            ((air_loc / exp(alpha + beta) - 1 <= +prec) && (air_loc / exp(beta - alpha) - 1 >= -prec)) ? true : false;
        return res;
    }
