...
namespace Kadath
{
    void Domain_nucleus::export_tau_val_domain_boundary_vr(const Val_domain& so, int bound, Array<double>& sec,
            ...
            for (int k = 4; k < nbr_coefs(2) - 1; k++) {
                ...
                if (mquant % 2 == 0) {
                    ...
                    for (int j = 1; j < nbr_coefs(1); j++) {
                        ...
                        sec.set(pos_sec) = val_boundary(bound, so, pos_cf) - 2. * val_boundary(bound, so, pos_galerkin);
                        pos_sec++;
                    }
                }
    ...
    void Domain_nucleus::export_tau_val_domain_boundary_vt(const Val_domain& so, int bound, Array<double>& sec,
                                                           int& pos_sec, int ncond) const
    {
            ...
            for (int k = 4; k < nbr_coefs(2) - 1; k++) {
                ...
                if (mquant % 2 == 1) {
                    ...
                    for (int j = 1; j < nbr_coefs(1) - 1; j++) {
                        ...
                        pos_galerkin = pos_cf;
                        pos_galerkin.set(1) = 0;
                        sec.set(pos_sec) = val_boundary(bound, so, pos_cf) - val_boundary(bound, so, pos_galerkin);
                        pos_sec++;
                    }
                }
            }
    ...
    void Domain_nucleus::export_tau_val_domain_boundary_vp(const Val_domain& so, int bound, Array<double>& sec,
                                                           int& pos_sec, int ncond) const
    {
    ...
    void Domain_nucleus::export_tau_val_domain_boundary(const Val_domain& so, int mlim, int bound, Array<double>& sec,
                                                        int& pos_sec, int ncond) const
    {
                    ...
                    for (int j = 0; j < nbr_coefs(1); j++) {
                        ...
                        switch (baset) {
                            ...
                            case SIN_ODD:
                                ...
                                break;
                            default:
                                KADATH_THROW("Unknow theta basis in Domain_nucleus::export_tau_val_domain_boundary");
                        }
                    }
                }
        }
    ...
    void Domain_nucleus::export_tau_boundary(const Tensor& tt, int dom, int bound, Array<double>& res, int& pos_res,
                                             const Array<int>& ncond, int n_cmp, Array<int>** p_cmp) const
    {

        // Check boundary
        if (bound != OUTER_BC) {
            KADATH_THROW("Unknown boundary in Domain_nucleus::export_tau_boundary");
        }
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (tt.get_basis().get_basis(dom) == SPHERICAL_BASIS) {
                    if (n_cmp == -1) {
                        export_tau_val_domain_boundary_vr(tt(1)(dom), bound, res, pos_res, ncond(0));
                        export_tau_val_domain_boundary_vt(tt(2)(dom), bound, res, pos_res, ncond(1));
                        export_tau_val_domain_boundary_vp(tt(3)(dom), bound, res, pos_res, ncond(2));
                    } else
                        for (int i = 0; i < n_cmp; i++) {
                            if ((*p_cmp[i])(0) == 1)
                                export_tau_val_domain_boundary_vr(tt(1)(dom), bound, res, pos_res, ncond(i));
                            if ((*p_cmp[i])(0) == 2)
                                export_tau_val_domain_boundary_vt(tt(2)(dom), bound, res, pos_res, ncond(i));
                            if ((*p_cmp[i])(0) == 3)
                                export_tau_val_domain_boundary_vp(tt(3)(dom), bound, res, pos_res, ncond(i));
                        }
                    found = true;
                }
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_nucleus::export_tau_boundary");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_nucleus::export_tau_boundary");
                }
            } break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_nucleus::export_tau_boundary" << endl;
                break;
        }
    }
} // namespace Kadath
