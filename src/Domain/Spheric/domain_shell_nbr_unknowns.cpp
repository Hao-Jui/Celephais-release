...
namespace Kadath
{
    int Domain_shell::nbr_unknowns_val_domain(const Val_domain& so, int mlim) const
    {
        ...
        do {
            ...
            switch (baset) {
                ...
                case SIN_ODD:
                    ...
                    break;
                default:
                    KADATH_THROW("Unknow theta basis in Domain_shell::nbr_unknowns_val_domain");
            }
            if (indic)
                res++;
        } while (pos.inc());
    ...
    int Domain_shell::nbr_unknowns(const Tensor& tt, int dom) const
    {
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_shell::nbr_unknowns");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_shell::nbr_unknowns");
                }
            } break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_shell::nbr_unknowns" << endl;
                break;
        }
        return res;
    }
