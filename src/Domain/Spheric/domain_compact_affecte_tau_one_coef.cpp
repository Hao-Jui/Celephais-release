...
namespace Kadath
{
    namespace
    {
        void append_compact_tau_seed_component(
            ...
            for (int k = 0; k < nbr_coefs(2) - 1; ++k) {
                ...
                for (int j = 0; j < nbr_coefs(1); ++j) {
                    ...
                    switch (baset) {
                        ...
                        default:
                            KADATH_THROW(
                                "Unknown theta basis in Domain_compact::describe_tau_seed_block");
                    }
                    if (!true_theta)
                        continue;
    ...
    void Domain_compact::affecte_tau_one_coef_val_domain(Val_domain& so, int mlim, int cc, int& conte) const
    {
                ...
                for (int j = 0; j < nbr_coefs(1); j++) {
                    ...
                    switch (baset) {
                        ...
                        case SIN_ODD:
                            ...
                            break;
                        default:
                            KADATH_THROW("Unknow theta basis in Domain_compact::affecte_one_coef_val_domain");
                    }
    ...
    void Domain_compact::affecte_tau_one_coef(Tensor& tt, int dom, int cc, int& pos_cf) const
    {
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_compact::affecte_tau_one_coef");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_compact::affecte_tau_one_coef");
                }
            } break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_compact::affecte_tau" << endl;
                break;
        }
    }

    bool Domain_compact::describe_tau_seed_block(
        const Tensor& tt, int dom, std::vector<TauSeedDescriptor>& descriptors) const
    {
        ...
        for (int component = 0; component < tt.get_n_comp(); ++component) {
            ...
            append_compact_tau_seed_component(
