...
namespace Kadath
{
    void Domain_shell::export_tau_val_domain_boundary(const Val_domain& so, int mlim, int bound, Array<double>& sec,
                    ...
                    for (int j = 0; j < nbr_coefs(1); j++) {
                        ...
                        switch (baset) {
                            ...
                            case SIN_ODD:
                                ...
                                break;
                            default:
                                KADATH_THROW("Unknow theta basis in Domain_shell::export_tau_val_domain_boundary");
                        }
                    }
                }
        }
    ...
    void Domain_shell::export_tau_boundary(const Tensor& tt, int dom, int bound, Array<double>& res, int& pos_res,
                                           const Array<int>& ncond, int n_cmp, Array<int>** p_cmp) const
    {

        // Check boundary
        if ((bound != INNER_BC) && (bound != OUTER_BC)) {
            KADATH_THROW("Unknown boundary in Domain_shell::export_tau_boundary");
        }
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_shell::export_tau_boundary");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_shell::export_tau_boundary");
                }
            } break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_shell::export_tau_boundary" << endl;
                break;
        }
    }
} // namespace Kadath
