...
namespace Kadath
{
    Term_eq Domain_shell::derive_flat_spher(int type_der, char ind_der, const Term_eq& so,
                                            const Metric* manipulator) const
    {
        ...
        do {
            ...
            switch (pos_auxi_bis(0)) {
                ...
                case 1:
                    // 1/r dtheta
                    auxi_val.set(pos_auxi_bis).set_domain(num_dom) =
                        (*so.val_t)(pos_so_bis)(num_dom).der_var(2).div_r();
                    break;
                case 2:
                    // 1/r sint d/dphi
                    auxi_val.set(pos_auxi_bis).set_domain(num_dom) =
                        (*so.val_t)(pos_so_bis)(num_dom).der_var(3).div_r().div_sin_theta();
                    break;
                default:
                    KADATH_THROW("Bad indice in Domain_shell::derive_flat_spher");
            }
                        ...
                        switch (pos_auxi(ind_sum + 1)) {
                            ...
                            case 1:
                                ...
                                auxi_val.set(pos_auxi).set_domain(num_dom) -=
                                    (*so.val_t)(pos_so)(num_dom).div_r().mult_cos_theta().div_sin_theta();
                                break;
                            case 2:
                                // Dphi S_phi
                                pos_so.set(ind_sum) = 0;
        ...
        if (so.der_t == nullptr) {
            ...
            do {
                ...
                switch (pos_auxi_der_bis(0)) {
                    ...
                    case 1:
                        // 1/r dtheta
                        auxi_der.set(pos_auxi_der_bis).set_domain(num_dom) =
                            (*so.der_t)(pos_so_bis)(num_dom).der_var(2).div_r();
                        break;
                    case 2:
                        // 1/r sint d/dphi
                        auxi_der.set(pos_auxi_der_bis).set_domain(num_dom) =
                            (*so.der_t)(pos_so_bis)(num_dom).der_var(3).div_r().div_sin_theta();
                        break;
                    default:
                        KADATH_THROW("Bad indice in Domain_shell::derive_flat_spher");
                }
                        ...
                        case 2:
                            ...
                            switch (pos_auxi_der(ind_sum + 1)) {
                                ...
                                case 1:
                                    ...
                                    pos_so.set(ind_sum) = 2;
                                    auxi_der.set(pos_auxi_der).set_domain(num_dom) -=
                                        (*so.der_t)(pos_so)(num_dom).div_r().mult_cos_theta().div_sin_theta();
                                    break;
                                case 2:
                                    // Dphi S_phi
                                    pos_so.set(ind_sum) = 0;
    ...
    Term_eq Domain_shell::derive_flat_cart(int type_der, char ind_der, const Term_eq& so,
                                           const Metric* manipulator) const
    {
        ...
        if (!doder) {
            ...
            do {
                for (int i = 0; i < val_res - 1; i++)
                    pos_so.set(i) = pos_auxi_der(i + 1);
                auxi_der.set(pos_auxi_der).set_domain(num_dom) =
                    (*so.der_t)(pos_so)(num_dom).der_abs(pos_auxi_der(0) + 1);
            } while (pos_auxi_der.inc());
    ...
    Term_eq Domain_shell::derive_flat_mtz(int type_der, char ind_der, const Term_eq& so,
                                          const Metric* manipulator) const
    {
        ...
        // Tensor for val
        Base_tensor basis(so.val_t->get_space(), MTZ_BASIS);
        Tensor auxi_val(so.val_t->get_space(), val_res, type_ind, basis);
        ...
        do {
            ...
            switch (pos_auxi_bis(0)) {
                ...
                case 1:
                    // cost/r dtheta
                    auxi_val.set(pos_auxi_bis).set_domain(num_dom) =
                        (*so.val_t)(pos_so_bis)(num_dom).der_var(2).div_r().mult_cos_theta();
                    break;
                case 2:
                    // cost/r sint d/dphi
                    auxi_val.set(pos_auxi_bis).set_domain(num_dom) =
                        (*so.val_t)(pos_so_bis)(num_dom).der_var(3).div_r().div_sin_theta().mult_cos_theta();
                    break;
                default:
                    KADATH_THROW("Bad indice in Domain_shell::derive_flat_mtz");
            }
        } while (pos_auxi_bis.inc());
                    ...
                    case 2:
                        ...
                        switch (pos_auxi(ind_sum + 1)) {
                            ...
                            case 1:
                                ...
                                pos_so.set(ind_sum) = 2;
                                auxi_val.set(pos_auxi).set_domain(num_dom) -=
                                    (*so.val_t)(pos_so)(num_dom).div_r().div_sin_theta();
                                break;
                            case 2:
                                // Dphi S_phi
                                pos_so.set(ind_sum) = 0;
            ...
            do {
                ...
                switch (pos_auxi_der_bis(0)) {
                    ...
                    case 1:
                        // cost/r dtheta
                        auxi_der.set(pos_auxi_der_bis).set_domain(num_dom) =
                            (*so.der_t)(pos_so_bis)(num_dom).der_var(2).div_r().mult_cos_theta();
                        break;
                    case 2:
                        // cost/r sint d/dphi
                        auxi_der.set(pos_auxi_der_bis).set_domain(num_dom) =
                            (*so.der_t)(pos_so_bis)(num_dom).der_var(3).div_r().div_sin_theta().mult_cos_theta();
                        break;
                    default:
                        KADATH_THROW("Bad indice in Domain_shell::derive_flat_mtz");
                }
                        ...
                        case 2:
                            ...
                            switch (pos_auxi_der(ind_sum + 1)) {
                                ...
                                case 1:
                                    ...
                                    pos_so.set(ind_sum) = 2;
                                    auxi_der.set(pos_auxi_der).set_domain(num_dom) -=
                                        (*so.der_t)(pos_so)(num_dom).div_r().div_sin_theta();
                                    break;
                                case 2:
                                    // Dphi S_phi
                                    pos_so.set(ind_sum) = 0;
