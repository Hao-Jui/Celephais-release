...
namespace Kadath
{
    Tensor Domain_shell::change_basis_spher_to_cart(int dd, const Tensor& so) const
    {

        // Lust start from spherical tensorial basis
        if (so.get_basis().get_basis(dd) != SPHERICAL_BASIS) {
            KADATH_THROW("The input tensorial basis must be spherical in Domain_shell::change_basis_spher_to_cart");
        }
    ...
    Tensor Domain_shell::change_basis_cart_to_spher(int dd, const Tensor& so) const
    {
        // Must start from spherical tensorial basis
        if (so.get_basis().get_basis(dd) != CARTESIAN_BASIS) {
            KADATH_THROW("The input tensorial basis must be cartesian in Domain_shell::change_basis_cart_to_spher");
        }
        // Need to remove the symetry :
        int val = so.get_valence();
        Array<int> type_ind(so.get_index_type());
