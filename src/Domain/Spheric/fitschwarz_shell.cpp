...
namespace Kadath
{
    ...
    // Term_eq version
    Term_eq Domain_shell::fitschwarz(const Term_eq& so, int dim) const
    {

        // Check it is a tensor
        if (so.get_type_data() != TERM_T) {
            KADATH_THROW("fitschwarz only defined with respect for a tensor");
        }

        // Right domain ?
        int dom = so.get_dom();
        if (this != so.get_val_t().get_space().get_domain(dom)) {
            KADATH_THROW("Domain mismatch in Domain_shell::fitschwarz (Term_eq version)");
        }
