...
namespace Kadath
{
    ...
    Domain_shell::Domain_shell(int num, int ttype, double rint, double rext, const Point& cr, const Dim_array& nbr)
    ...
    // Constructor by copy
    Domain_shell::Domain_shell(const Domain_shell& so) : Domain(so), alpha(so.alpha), beta(so.beta), center(so.center)
    {
    }

    Domain_shell::Domain_shell(int num, BinarySource& source)
    ...
    void Domain_shell::save(BinarySink& sink) const
    {
    ...
    Val_domain Domain_shell::der_normal(const Val_domain& so, int bound) const
    {
        ...
        switch (bound) {
            ...
            case INNER_BC:
                ...
                break;
            default:
                KADATH_THROW("Unknown boundary case in Domain_shell::der_normal");
        }
        return res;
    }
    ...
    void Domain_shell::do_coloc()
    {
        ...
        switch (type_base) {
            ...
            case LEG_TYPE:
                ...
                break;
            default:
                KADATH_THROW("Unknown type of basis in Domain_shell::do_coloc");
        }
    }
