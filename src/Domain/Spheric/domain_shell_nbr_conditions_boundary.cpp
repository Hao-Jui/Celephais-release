...
namespace Kadath
{
            ...
            for (int j = 0; j < nbr_coefs(1); j++) {
                ...
                switch (baset) {
                    ...
                    case SIN_ODD:
                        ...
                        break;
                    default:
                        KADATH_THROW("Unknow theta basis in Domain_shell::nbr_conditions_val_boundary");
                }
    ...
    Array<int> Domain_shell::nbr_conditions_boundary(const Tensor& tt, int dom, int bound, int n_cmp,
                                                     Array<int>** p_cmp) const
    {

        // Check boundary
        if ((bound != INNER_BC) && (bound != OUTER_BC)) {
            KADATH_THROW("Unknown boundary in Domain_shell::nbr_conditions_boundary");
        }
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_shell::nbr_conditions_boundary");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_shell::nbr_conditions_boundary");
                }
            } break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_shell::nbr_conditions_boundary" << endl;
                break;
        }
        return res;
    }

    bool Domain_shell::describe_boundary_residual_rows(
