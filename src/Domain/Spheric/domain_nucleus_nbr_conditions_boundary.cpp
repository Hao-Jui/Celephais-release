...
namespace Kadath
{
    int Domain_nucleus::nbr_conditions_val_domain_boundary_vr(const Val_domain& so) const
    {
            ...
            for (int j = 0; j < nbr_coefs(1); j++) {
                ...
                switch (baset) {
                    ...
                    case SIN_ODD:
                        ...
                        break;
                    default:
                        KADATH_THROW("Unknow theta basis in Domain_nucleus::nbr_conditions_val_domain_boundary_vr");
                }
    ...
    int Domain_nucleus::nbr_conditions_val_domain_boundary_vt(const Val_domain& so) const
    {
            ...
            for (int j = 0; j < nbr_coefs(1); j++) {
                ...
                switch (baset) {
                    ...
                    case COS_ODD:
                        ...
                        break;
                    default:
                        KADATH_THROW("Unknow theta basis in Domain_nucleus::nbr_conditions_val_domain_boundary_vt");
                }
    ...
    int Domain_nucleus::nbr_conditions_val_domain_boundary_vp(const Val_domain& so) const
    {
            ...
            for (int j = 0; j < nbr_coefs(1); j++) {
                ...
                switch (baset) {
                    ...
                    case SIN_ODD:
                        ...
                        break;
                    default:
                        KADATH_THROW("Unknow theta basis in Domain_nucleus::nbr_conditions_val_domain_boundary_vp");
                }
    ...
    int Domain_nucleus::nbr_conditions_val_domain_boundary(const Val_domain& so, int mlim) const
    {
            ...
            for (int j = 0; j < nbr_coefs(1); j++) {
                ...
                switch (baset) {
                    ...
                    case SIN_ODD:
                        ...
                        break;
                    default:
                        KADATH_THROW("Unknow theta basis in Domain_nucleus::nbr_conditions_val_boundary");
                }
    ...
    Array<int> Domain_nucleus::nbr_conditions_boundary(const Tensor& tt, int dom, int bound, int n_cmp,
                                                       Array<int>** p_cmp) const
    {

        // Check boundary
        if (bound != OUTER_BC) {
            KADATH_THROW("Unknown boundary in Domain_nucleus::nbr_conditions_boundary");
        }
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (tt.get_basis().get_basis(dom) == SPHERICAL_BASIS) {
                    if (n_cmp == -1) {
                        res.set(0) = nbr_conditions_val_domain_boundary_vr(tt(1)(dom));
                        res.set(1) = nbr_conditions_val_domain_boundary_vt(tt(2)(dom));
                        res.set(2) = nbr_conditions_val_domain_boundary_vp(tt(3)(dom));
                    } else
                        for (int i = 0; i < n_cmp; i++) {
                            if ((*p_cmp[i])(0) == 1)
                                res.set(i) = nbr_conditions_val_domain_boundary_vr(tt(1)(dom));
                            if ((*p_cmp[i])(0) == 2)
                                res.set(i) = nbr_conditions_val_domain_boundary_vt(tt(2)(dom));
                            if ((*p_cmp[i])(0) == 3)
                                res.set(i) = nbr_conditions_val_domain_boundary_vp(tt(3)(dom));
                        }
                    found = true;
                }
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_nucleus::nbr_conditions_boundary");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_nucleus::nbr_conditions_boundary");
                }
            } break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_nucleus::nbr_conditions_boundary" << endl;
                break;
        }
        return res;
    }

    bool Domain_nucleus::describe_boundary_residual_rows(
