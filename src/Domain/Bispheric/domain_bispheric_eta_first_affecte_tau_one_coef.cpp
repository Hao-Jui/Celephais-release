...
namespace Kadath
{
    namespace
    {
        bool append_bispheric_eta_first_tau_seed_component(
            ...
            std::vector<TauSeedDescriptor>& descriptors)
        {
            ...
            do {
                ...
                const int eta_basis =
                    (*field.get_base().get_base_1d(0))(chi, phi);
                const bool even_eta =
                    eta_basis == CHEB_EVEN || eta_basis == LEG_EVEN;
                const bool odd_eta =
                    eta_basis == CHEB_ODD || eta_basis == LEG_ODD;
                if ((phi % 2 == 0 && !even_eta) ||
                    (phi % 2 != 0 && !odd_eta)) {
                    return false;
                }
                ...
                bool include = true;
                if (phi_basis == COS) {
                    if ((phi % 2 != 0 && eta == nbr_coefs(0) - 1) ||
                        (phi != 0 && phi % 2 == 0 && eta == 0)) {
                        include = false;
                    }
                } else {
                    if (phi == 0 || phi == nbr_coefs(2) - 1 ||
                        (phi % 2 != 0 && eta == nbr_coefs(0) - 1) ||
                        (phi % 2 == 0 && eta == 0)) {
                        include = false;
                    }
                }
        ...
        do {
            ...
            switch (basep) {
                ...
                case SIN:
                    ...
                    break;
                default:
                    KADATH_THROW("Unknwon phi basis in Domain_bispheric_eta_first::export_tau_one_coef_val_domain");
            }
    ...
    void Domain_bispheric_eta_first::affecte_tau_one_coef(Tensor& tt, int dom, int cc, int& pos_cf) const
    {
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_bispheric_eta_first::affecte_tau_one_coef");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_bispheric_eta_first::affecte_tau_one_coef");
                }
            } break;
            default:
    ...
    bool Domain_bispheric_eta_first::describe_tau_seed_block(
        ...
        std::vector<TauSeedDescriptor>& descriptors) const
    {
        ...
        for (int component = 0; component < tt.get_n_comp(); ++component) {
            ...
            if (!append_bispheric_eta_first_tau_seed_component(
