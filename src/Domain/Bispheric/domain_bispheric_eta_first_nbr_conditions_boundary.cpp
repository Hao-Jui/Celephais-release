...
namespace Kadath
{
    int Domain_bispheric_eta_first::nbr_conditions_val_domain_boundary(const Val_domain& so, int bound) const
    {
                ...
                for (int i = 0; i < nbr_coefs(0); i++) {
                    ...
                    switch (basep) {
                        ...
                        case SIN:
                            ...
                            break;
                        default:
                            KADATH_THROW("Unknwon phi basis in Domain_bispheric_eta_first:nbr_conditions_tau_boundary");
                    }
    ...
    Array<int> Domain_bispheric_eta_first::nbr_conditions_boundary(const Tensor& tt, int dom, int bound, int n_cmp,
                                                                   Array<int>** p_cmp) const
    {

        // Check boundary
        if ((bound != ETA_MINUS_BC) && (bound != ETA_PLUS_BC) && (bound != OUTER_BC)) {
            KADATH_THROW("Unknown boundary in Domain_bispheric_eta_first::nbr_conditions_boundary");
        }
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_bispheric_eta_first::nbr_conditions_boundary");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_bispheric_eta_first::nbr_conditions_boundary");
                }
            } break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_bispheric_eta_first::nbr_conditions_boundary"
                     << endl;
                break;
        }
        return res;
    ...
    bool Domain_bispheric_eta_first::describe_boundary_residual_rows(
