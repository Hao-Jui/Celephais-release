...
namespace Kadath
{
    ...
    Domain_bispheric_rect::Domain_bispheric_rect(int num, int ttype, double a, double rr, double etamin, double etapl,
                                                 double chimin, const Dim_array& nbr)
        : Domain(num, ttype, nbr), aa(a), r_ext(rr), eta_minus(etamin), eta_plus(etapl), chi_min(chimin),
          p_eta(nullptr), p_chi(nullptr), p_phi(nullptr), p_detadx(nullptr), p_detady(nullptr), p_detadz(nullptr),
          p_dchidx(nullptr), p_dchidy(nullptr), p_dchidz(nullptr), p_dphidy(nullptr), p_dphidz(nullptr),
          p_dsint(nullptr)
    {
    ...
    // Copy constructor
    Domain_bispheric_rect::Domain_bispheric_rect(const Domain_bispheric_rect& so)
        : Domain(so), aa(so.aa), eta_minus(so.eta_minus), eta_plus(so.eta_plus), chi_min(so.chi_min)
    {
    ...
    Domain_bispheric_rect::Domain_bispheric_rect(int num, BinarySource& source) : Domain(num, source)
    {
    ...
    int Domain_bispheric_rect::phi_coefficient_parity(
    ...
    void Domain_bispheric_rect::save(BinarySink& sink) const
    {
    ...
    const Point Domain_bispheric_rect::absol_to_num_bound(const Point& abs, int bound) const
    {
        ...
        switch (bound) {
            ...
            case OUTER_BC:
                ...
                break;
            default:
                KADATH_THROW("Unknown case in Domain_bispheric_rect::absol_to_num_bound");
        }
        return num;
    }
    ...
    Val_domain Domain_bispheric_rect::der_normal(const Val_domain& so, int bound) const
    {
        ...
        switch (bound) {
            ...
            default:
                KADATH_THROW("Unknown boundary case in Domain_bispheric_rect::der_normal");
        }
    }
    ...
    double Domain_bispheric_rect::integ(const Val_domain& so, int bound) const
    {
        ...
        if (bound != INNER_BC) {
            KADATH_THROW("Domain_bispheric_rect::integ only defined for inner boundary yet");
        }
                ...
                for (int j = 0; j < nbr_coefs(1); j++) {
                    ...
                    switch (base_chi) {
                        ...
                        case CHEB_ODD:
                            ...
                            break;
                        default:
                            KADATH_THROW("Unknown basis in Domain_bispheric_rect::integ");
                    }
