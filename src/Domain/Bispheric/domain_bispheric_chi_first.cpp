...
namespace Kadath
{
    ...
    Domain_bispheric_chi_first::Domain_bispheric_chi_first(int num, int ttype, double a, double etalim, double air,
                                                           double chim, const Dim_array& nbr)
        : Domain(num, ttype, nbr), aa(a), eta_lim(etalim), r_ext(air), chi_max(chim), bound_eta(nullptr),
          bound_eta_der(nullptr), p_eta(nullptr), p_chi(nullptr), p_phi(nullptr), p_detadx(nullptr), p_detady(nullptr),
    ...
    Domain_bispheric_chi_first::Domain_bispheric_chi_first(int num, BinarySource& source)
        : Domain(num, source)
    {
    ...
    int Domain_bispheric_chi_first::phi_coefficient_parity(
        int k, int phi_basis) const
    {
        const int harmonic_parity = (k % 2) ? -1 : +1;
        if (phi_basis == COS)
            return harmonic_parity;
        if (phi_basis == SIN)
            return -harmonic_parity;
        return 0;
    }

    void Domain_bispheric_chi_first::save(BinarySink& sink) const
    {
    ...
    void Domain_bispheric_chi_first::do_radius() const
    {
        ...
        radius =
            new Val_domain(sqrt(get_cart(1) * get_cart(1) + get_cart(2) * get_cart(2) + get_cart(3) * get_cart(3)));
    }
    ...
    const Point Domain_bispheric_chi_first::absol_to_num_bound(const Point& abs, int bound) const
    {
        ...
        switch (bound) {
            ...
            case CHI_ONE_BC:
                ...
                break;
            default:
                KADATH_THROW("Unknown case in Domain_bispheric_chi_first::absol_to_num_bound");
        }
    ...
    void Domain_bispheric_chi_first::do_coloc()
    {
        ...
        switch (type_base) {
            ...
            case LEG_TYPE:
                ...
                break;
            default:
                KADATH_THROW("Unknown type of basis in Domain_bispheric_rect::do_coloc");
        }
    }
    ...
    Val_domain Domain_bispheric_chi_first::der_normal(const Val_domain& so, int bound) const
    {
        ...
        switch (bound) {
            ...
            default:
                KADATH_THROW("Unknown boundary case in Domain_bispheric_chi_first::der_normal");
        }
    }
    ...
    double Domain_bispheric_chi_first::integ(const Val_domain& so, int bound) const
    {
        ...
        if (bound != INNER_BC) {
            KADATH_THROW("Domain_bispheric_chi_first::integ only defined for inner boundary yet..");
        }
                ...
                for (int j = 0; j < nbr_coefs(1); j++) {
                    ...
                    switch (base_chi) {
                        ...
                        case CHEB_ODD:
                            ...
                            break;
                        default:
                            KADATH_THROW("Unknown basis in Domain_bispheric_chi_first::integ");
                    }
