...
{
    // Tensorial parts :
    Tensor Domain_bispheric_eta_first::import(int numdom, int bound, int n_ope, const Array<int>& zedoms,
                                              Tensor** parts) const
    {
        ...
        switch (bound) {
            case OUTER_BC:
                index_chi = nbr_points(0) - 1;
                break;
            default:
                cerr << "Unknown boundary in Domain_bispheric_eta_first::import" << endl;
        }
        ...
        for (int k = 0; k < nbr_points(2); k++)
            for (int j = 0; j < nbr_points(1); j++) {
                ...
                while ((current < n_ope) && (!found)) {
                    if (parts[0]->get_space().get_domain(zedoms(current))->is_in(MM, 1e-3))
                        found = true;
                    else
                        current++;
                }
                if (!found) {
                    ...
                    oss << "Point " << MM << " not found in other domains, for Domain_bispheric_eta_first::import"
                         << endl;
