...
{
    // Tensorial parts :
    Tensor Domain_bispheric_chi_first::import(int numdom, int bound, int n_ope, const Array<int>& zedoms,
                                              Tensor** parts) const
    {
        ...
        switch (bound) {
            case INNER_BC:
                ...
                break;
            case OUTER_BC:
                index_mu = nbr_points(0) - 1;
                break;
            default:
                cerr << "Unknown boundary in Domain_bispheric_chi_first::import" << endl;
        }
        ...
        for (int k = 0; k < nbr_points(2); k++)
            for (int j = 0; j < nbr_points(1); j++) {
                ...
                if (!found) {
                    ...
                    oss << "Point " << MM << " not found in other domains, for Domain_bispheric_chi_first::import"
                         << endl;
