...
namespace Kadath
{
    ...
    Array<int> Domain_bispheric_eta_first::nbr_conditions_boundary_one_side(const Tensor& tt, int dom, int bound,
                                                                            int n_cmp, Array<int>** p_cmp) const
    {

        // Check boundary
        if ((bound != ETA_MINUS_BC) && (bound != ETA_PLUS_BC) && (bound != OUTER_BC)) {
            KADATH_THROW("Unknown boundary in Domain_bispheric_eta_first::nbr_conditions_boundary_one_side");
        }
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_bispheric_eta_first::nbr_conditions_boundary_one_side");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_bispheric_eta_first::nbr_conditions_boundary_one_side");
                }
            } break;
            default:
                cerr << "Valence " << val
                     << " not implemented in Domain_bispheric_eta_first::nbr_conditions_boundary_one_side" << endl;
                break;
        }
        return res;
    }
