...
namespace Kadath
{
    int Domain_bispheric_eta_first::nbr_conditions_val_domain(const Val_domain& so, int order) const
    {
        ...
        switch (order) {
            ...
            case 2:
                ...
                break;
            default:
                cerr << "Unknown order in Domain_bispheric_eta_first::nbr_conditions_val_domain" << endl;
                break;
        }
        ...
        for (int k = 0; k < nbr_coefs(2); k++)
            ...
            for (int i = 0; i < nbr_coefs(0); i++) {
                ...
                switch (basep) {
                    case COS:
                        // Last odd ones
                        if ((k % 2 == 1) && (i == nbr_coefs(0) - 1 - forgot_chi))
                            true_other = false;
                        // Regularity for even ones :
                        if ((k != 0) && (k % 2 == 0) && (i == 0))
                            true_other = false;
                        if (i == nbr_coefs(1) - forgot_chi)
                            true_other = false;
                        break;
                    case SIN:
                        // sin(0)
                        ...
                        if ((k % 2 == 0) && (i == 0))
                            true_other = false;
                        if (i == nbr_coefs(0) - forgot_chi)
                            true_other = false;
                        break;
                    default:
                        KADATH_THROW("Unknwon phi basis in Domain_bispheric_eta_first:nbr_conditions_val_domain");
                }

                if (true_other)
                    res += nbr_coefs(0) - forgot_eta;
            }
        return res;
    }

    Array<int> Domain_bispheric_eta_first::nbr_conditions(const Tensor& tt, int dom, int order, int n_cmp,
                                                          Array<int>** p_cmp) const
    {
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_bispheric_eta_first::nbr_conditions");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_bispheric_eta_first::nbr_conditions");
                }
            } break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_bispheric_eta_first::nbr_conditions" << endl;
                break;
        }
        return res;
    }

    bool Domain_bispheric_eta_first::describe_volume_residual_rows(
        ...
        for (std::size_t slot = 0; slot < components.size(); ++slot) {
            ...
            for (int k = 0; k < nbr_coefs(2); ++k) {
                for (int j = 0; j < nbr_coefs(1) - forgot_eta; ++j) {
                    for (int i = 0; i < nbr_coefs(0) - forgot_chi; ++i) {
                        bool emitted = true;
                        if (phi_basis == COS) {
                            emitted = !((k % 2 == 1) &&
                                        (i == nbr_coefs(0) - 1 - forgot_chi));
                            emitted = emitted &&
                                      (k == 0 || k % 2 == 1 || i != 0);
                        } else {
