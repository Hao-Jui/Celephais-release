...
namespace Kadath
{
    int Domain_shell_outer_adapted_nosym::nbr_conditions_val_domain_boundary(const Val_domain& so, int mlim) const
    {
        ...
        if (!count_base->is_def() || count_base->get_base_1d(1) == nullptr) {
            switch (get_type_base()) {
                ...
                case LEG_TYPE:
                    ...
                    break;
                default:
                    KADATH_THROW("Unknown type of base in Domain_shell_outer_adapted_nosym::nbr_conditions_val_domain_boundary");
            }
        ...
        for (int k = 0; k < nbr_coefs(2); k++)
            for (int j = 0; j < nbr_coefs(1); j++) {
                ...
                if ((k == 1) || (k == nbr_coefs(2) - 1))
                    indic = false;
                // Get base in theta :
                int baset = (*count_base->get_base_1d(1))(k);
    ...
    Array<int> Domain_shell_outer_adapted_nosym::nbr_conditions_boundary(const Tensor& tt, int dom, int bound, int n_cmp,
                                                                   Array<int>** p_cmp) const
    {

        // Check boundary
        if ((bound != INNER_BC) && (bound != OUTER_BC)) {
            KADATH_THROW("Unknown boundary in Domain_shell_outer_adapted_nosym::nbr_conditions_boundary");
        }
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_shell_outer_adapted_nosym::nbr_conditions_boundary");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_shell_outer_adapted_nosym::nbr_conditions_boundary");
                }
            } break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_shell_outer_adapted_nosym::nbr_conditions_boundary"
                     << endl;
                break;
        }
        return res;
    ...
    bool Domain_shell_outer_adapted_nosym::describe_boundary_residual_rows(
