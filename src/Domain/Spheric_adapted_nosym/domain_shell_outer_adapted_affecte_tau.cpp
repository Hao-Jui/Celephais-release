...
namespace Kadath
{
    void Domain_shell_outer_adapted_nosym::affecte_tau_val_domain(Val_domain& so, int mlim, const Array<double>& values,
                                                            int& conte) const
    {
            ...
            for (int i = 0; i < nbr_coefs(0); i++) {
                ...
                switch (baset) {
                    ...
                    case SIN:
                        ...
                        break;
                    default:
                        KADATH_THROW("Unknow theta basis in Domain_shell_outer_adapted_nosym::affecte_tau_val_domain");
                }
            }
        }
    }

    void Domain_shell_outer_adapted_nosym::affecte_tau(Tensor& tt, int dom, const Array<double>& cf, int& pos_cf) const
    {
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_shell_outer_adapted_nosym::affecte_tau");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_shell_outer_adapted_nosym::affecte_tau");
                }
            } break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_shell_outer_adapted_nosym::affecte_tau" << endl;
                break;
        }
    }
} // namespace Kadath
