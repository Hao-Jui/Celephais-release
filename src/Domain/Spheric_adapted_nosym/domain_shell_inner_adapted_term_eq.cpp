...
namespace Kadath
{
    Term_eq Domain_shell_inner_adapted_nosym::dr_term_eq(const Term_eq& so) const
    {
        return derive_r(so);
    }

    Term_eq Domain_shell_inner_adapted_nosym::derive_r(const Term_eq& so) const
    {
    ...
    Term_eq Domain_shell_inner_adapted_nosym::derive_t(const Term_eq& so) const
    {
    ...
    Term_eq Domain_shell_inner_adapted_nosym::derive_p(const Term_eq& so) const
    {
    ...
    Term_eq Domain_shell_inner_adapted_nosym::flat_grad_spher(const Term_eq& so) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::do_normal_spher() const
    {
    ...
    void Domain_shell_inner_adapted_nosym::do_normal_cart() const
    {
    ...
    Term_eq Domain_shell_inner_adapted_nosym::der_normal_term_eq(const Term_eq& so, int bound) const
    {
        ...
        switch (bound) {
            ...
            default:
                KADATH_THROW("Unknown boundary in Domain_shell_inner_adapted_nosym::der_normal");
        }
    }

    Term_eq Domain_shell_inner_adapted_nosym::lap_term_eq(const Term_eq& so, int mm) const
    {
        if (mm != 0) {
            KADATH_THROW("Domain_shell_inner_adapted_nosym::lap_term_eq not defined for m != 0 (for now)");
        }

        if (so.get_val_t().get_valence() != 0) {
            KADATH_THROW("Domain_shell_inner_adapted_nosym::lap_term_eq only defined for scalars");
        }
    ...
    Term_eq Domain_shell_inner_adapted_nosym::mult_r_term_eq(const Term_eq& so) const
    {

        return so * (*rad_term_eq);
    }

    void Domain_shell_inner_adapted_nosym::update_term_eq(Term_eq* so) const
    {
        update_term_eq_impl(so, false);
    }

    void Domain_shell_inner_adapted_nosym::accumulate_term_eq_mapping_derivative(
        Term_eq* so) const
    {
        update_term_eq_impl(so, true);
    }

    void Domain_shell_inner_adapted_nosym::update_term_eq_impl(
        Term_eq* so, bool accumulate) const
    {
        auto mapping_derivative = [&](int lane) {
            Tensor der(
                one_domain_storage, num_dom, *so->val_t, false);
            const Tensor& radius_derivative = inner_radius_term_eq->get_der_t(lane);
    ...
    Term_eq Domain_shell_inner_adapted_nosym::partial_spher(const Term_eq& so) const
    {
        int dom = so.get_dom();
        assert(dom == num_dom);
    ...
    Term_eq Domain_shell_inner_adapted_nosym::connection_spher(const Term_eq& so) const
    {
                    ...
                    case 1:
                        ...
                        switch (pos_auxi(ind_sum + 1)) {
                            ...
                            case 2:
                                ...
                                break;
                            default:
                                KADATH_THROW("Bad indice in Domain_shell_inner_adapted_nosym::connection_spher");
                        }
                        break;
                    case 2:
                        // Dphi
        ...
        if ((so.der_t == nullptr) || (inner_radius_term_eq->der_t == nullptr)) {
    ...
    Term_eq Domain_shell_inner_adapted_nosym::partial_cart(const Term_eq& so) const
    {
        int dom = so.get_dom();
        assert(dom == num_dom);
    ...
    const Term_eq* Domain_shell_inner_adapted_nosym::give_normal(int bound, int tipe) const
    {
    ...
    Term_eq Domain_shell_inner_adapted_nosym::integ_volume_term_eq(const Term_eq& target) const
    {
