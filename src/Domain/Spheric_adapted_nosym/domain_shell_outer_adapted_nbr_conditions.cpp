...
namespace Kadath
{
    int Domain_shell_outer_adapted_nosym::nbr_conditions_val_domain(const Val_domain& so, int mlim, int order) const
    {
        ...
        if (!count_base->is_def() || count_base->get_base_1d(1) == nullptr) {
            switch (get_type_base()) {
                ...
                case LEG_TYPE:
                    ...
                    break;
                default:
                    KADATH_THROW("Unknown type of base in Domain_shell_outer_adapted_nosym::nbr_conditions_val_domain");
            }
    ...
    Array<int> Domain_shell_outer_adapted_nosym::nbr_conditions(const Tensor& tt, int dom, int order, int n_cmp,
                                                          Array<int>** p_cmp) const
    {
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_shell_outer_adapted_nosym::nbr_conditions");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_shell_outer_adapted_nosym::nbr_conditions");
                }
            } break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_shell_outer_adapted_nosym::nbr_conditions" << endl;
                break;
        }
        return res;
    }

    bool Domain_shell_outer_adapted_nosym::describe_volume_residual_rows(
