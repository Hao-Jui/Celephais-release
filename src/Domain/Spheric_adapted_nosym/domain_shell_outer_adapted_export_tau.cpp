...
namespace Kadath
{
    void Domain_shell_outer_adapted_nosym::export_tau_val_domain(const Val_domain& so, int mlim, int order,
                        ...
                        for (int i = 0; i < nbr_coefs(0) - order; i++) {
                            ...
                            switch (baset) {
                                case COS:
                                    if (detail::spheric_nosym_true_theta_coef(baset, j, k, kmin, nbr_coefs(1))) {
                                        ...
                                        if (detail::spheric_nosym_uses_theta_galerkin(baset, k, kmin)) {
                                            ...
                                            result_data[pos_sec] -=
                                                detail::spheric_nosym_export_anchor_weight(baset, j) * coefficient_data[anchor_offset];
                                        }
                                        pos_sec++;
                                    }
                                    break;
                                case SIN:
                                    ...
                                    break;
                                default:
                                    KADATH_THROW("Unknow theta basis in Domain_shell_outer_adapted_nosym::export_tau_val_domain");
                            }
                        }
                    }
                }
    ...
    void Domain_shell_outer_adapted_nosym::export_tau(const Tensor& tt, int dom, int order, Array<double>& res, int& pos_res,
                                                const Array<int>& ncond, int n_cmp, Array<int>** p_cmp) const
    {
        ...
        switch (val) {
            ...
            case 1: {
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of vector Domain_shell_outer_adapted_nosym::export_tau");
                }
            } break;
            case 2: {
                bool found = false;
                ...
                if (!found) {
                    KADATH_THROW("Unknown type of 2-tensor Domain_shell_outer_adapted_nosym::export_tau");
                }
            } break;
            default:
                cerr << "Valence " << val << " not implemented in Domain_shell_outer_adapted_nosym::export_tau" << endl;
                break;
        }
    }
} // namespace Kadath
