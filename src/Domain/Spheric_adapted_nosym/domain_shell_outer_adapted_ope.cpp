...
namespace Kadath
{
    ...
    Val_domain Domain_shell_outer_adapted_nosym::mult_cos_phi(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
            ...
            do {
                switch ((*so.base.bases_1d[1])(index_t)) {
                    ...
                    case SIN:
                        ...
                        break;
                    default:
                        KADATH_THROW("Unknown case in Domain_shell_outer_adapted_nosym::mult_cos_phi");
                }
            } while (index_t.inc());
    ...
    Val_domain Domain_shell_outer_adapted_nosym::mult_sin_phi(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
            ...
            do {
                switch ((*so.base.bases_1d[1])(index_t)) {
                    ...
                    case SIN:
                        ...
                        break;
                    default:
                        KADATH_THROW("Unknown case in Domain_shell_outer_adapted_nosym::mult_sin_phi");
                }
            } while (index_t.inc());
            *res.base.bases_1d[0] = *so.base.bases_1d[0];
    ...
    Val_domain Domain_shell_outer_adapted_nosym::mult_cos_theta(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
    ...
    Val_domain Domain_shell_outer_adapted_nosym::mult_sin_theta(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
    ...
    Val_domain Domain_shell_outer_adapted_nosym::div_sin_theta(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
    ...
    Val_domain Domain_shell_outer_adapted_nosym::div_cos_theta(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
    ...
    Val_domain Domain_shell_outer_adapted_nosym::ddp(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
    ...
    Val_domain Domain_shell_outer_adapted_nosym::der_r(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
    ...
    Val_domain Domain_shell_outer_adapted_nosym::div_r(const Val_domain& so) const
    {
        if (so.check_if_zero())
            return so;
        else {
    ...
    Val_domain Domain_shell_outer_adapted_nosym::laplacian(const Val_domain& so, int m) const
    {
    ...
    Val_domain Domain_shell_outer_adapted_nosym::laplacian2(const Val_domain& so, int m) const
    {
        Val_domain derr(so.der_r());
        Val_domain dert(so.der_var(2));
    ...
    double Domain_shell_outer_adapted_nosym::integ_volume(const Val_domain& so) const
    {
