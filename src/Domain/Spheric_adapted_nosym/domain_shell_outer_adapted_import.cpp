...
namespace Kadath
{
    namespace
    {
        constexpr double adapted_nosym_import_lookup_precision = 1e-9;
    }

    // Tensorial parts :
    Tensor Domain_shell_outer_adapted_nosym::import(int numdom, int bound, int n_ope, const Array<int>& zedoms,
                                              Tensor** parts) const
    {
        ...
        switch (bound) {
            ...
            case OUTER_BC:
                ...
                break;
            default:
                cerr << "Unknown boundary in Domain_shell_outer_adapted_nosym::import" << endl;
        }
        ...
        for (int k = 0; k < nbr_points(2); k++)
            for (int j = 0; j < nbr_points(1); j++) {
                ...
                while ((current < n_ope) && (!found)) {
                    if (parts[0]->get_space().get_domain(zedoms(current))->is_in(MM, adapted_nosym_import_lookup_precision))
                        found = true;
                    else
                        current++;
                }
                if (!found) {
                    ...
                    oss << "Point " << MM << " not found in other domains, for Domain_shell_outer_adapted_nosym::import";
                ...
                for (int nc = 0; nc < res.get_n_comp(); nc++) {
                    double val = ((*parts[current]->cmp[nc])(zedoms(current)).check_if_zero())
                                     ? 0
                                     : (*parts[current]->cmp[nc])(zedoms(current))
                                           .get_base()
                                           .summation(num, *(*parts[current]->cmp[nc])(zedoms(current)).cf);
                    // Loop on radius :
