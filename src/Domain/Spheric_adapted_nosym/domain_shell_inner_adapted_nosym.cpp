...
namespace Kadath
{
    ...
    Domain_shell_inner_adapted_nosym::Domain_shell_inner_adapted_nosym(const Space& sss, int num, int ttype, double rin,
    ...
    Domain_shell_inner_adapted_nosym::Domain_shell_inner_adapted_nosym(const Space& sss, int num, int ttype, const Val_domain& rin,
                                                           double rout, const Point& cr, const Dim_array& nbr)
        : Domain(num, ttype, nbr), sp(sss), outer_radius(rout), center(cr)
    {
        assert(nbr.get_ndim() == 3);
    ...
    // Constructor by copy
    Domain_shell_inner_adapted_nosym::Domain_shell_inner_adapted_nosym(const Domain_shell_inner_adapted_nosym& so)
        : Domain(so), sp(so.sp), outer_radius(so.outer_radius), center(so.center)
    {
    ...
    Domain_shell_inner_adapted_nosym::Domain_shell_inner_adapted_nosym(const Space& sss, int num, BinarySource& source)
        : Domain(num, source), sp(sss), center(source)
    {
        outer_radius = source.read<double>();
    ...
    void Domain_shell_inner_adapted_nosym::do_radius() const
    {
        for (int i = 0; i < 3; i++)
        ...
        inner_radius->coef_i();
        const double* const inner_data = inner_radius->c->get_data();
        double* const radius_data = radius->c->get_data();
        for_each_physical_point(nbr_points, [&](int i, int, int, std::size_t offset) {
            radius_data[offset] = (outer_radius - inner_data[offset]) / 2. * (*coloc[0])(i) +
                                  (outer_radius + inner_data[offset]) / 2.;
        });
        radius->std_r_base();
    }

    // Destructor
    Domain_shell_inner_adapted_nosym::~Domain_shell_inner_adapted_nosym()
    {
    ...
    void Domain_shell_inner_adapted_nosym::del_deriv() const
    {
    ...
    int Domain_shell_inner_adapted_nosym::nbr_unknowns_from_adapted() const
    {
    ...
    void Domain_shell_inner_adapted_nosym::vars_to_terms() const
    {
    ...
    void Domain_shell_inner_adapted_nosym::affecte_coef(int& conte, int cc, bool& found) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::affecte_coef_lane(
        int& conte, int cc, int lane, int lane_count, bool& found) const
    {
        ...
        *auxi.cf = 0;
        ...
        if (lane == 0)
            inner_radius_term_eq->reset_derivative_tile(lane_count);
        if (found) {
            Scalar auxi_scal(sp);
            auxi_scal.set_domain(num_dom) = auxi;
            inner_radius_term_eq->set_der_t(lane, auxi_scal);
        } else {
            inner_radius_term_eq->set_der_zero(lane);
        }
        update();
    }

    void Domain_shell_inner_adapted_nosym::restore_scalar_derivative_lanes() const
    {
        inner_radius_term_eq->reset_derivative_tile(1);
        update();
    }

    void Domain_shell_inner_adapted_nosym::xx_to_vars_from_adapted(Val_domain& new_inner_radius, const Array<double>& xx,
                                                             int& pos) const
    {
        ...
        for (int k = 0; k < nbr_coefs(2); k++)
            if ((k != 1) && (k != nbr_coefs(2) - 1)) {
                ...
                for (int j = 0; j < nbr_coefs(1); j++) {
                    ...
                    if (detail::spheric_nosym_uses_theta_galerkin(baset, k, 2)) {
                        pos_cf.set(1) = detail::spheric_nosym_theta_anchor(baset, j);
                        new_inner_radius.cf->set(pos_cf) +=
    ...
    void Domain_shell_inner_adapted_nosym::update_mapping(const Val_domain& cor)
    {
    ...
    void Domain_shell_inner_adapted_nosym::set_mapping(const Val_domain& cor) const
    {
        *inner_radius = cor;
        clear_import_plan_cache();
    ...
    void Domain_shell_inner_adapted_nosym::update_variable(const Val_domain& cor_inner_radius, const Scalar& old,
                                                     Scalar& res) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::xx_to_ders_from_adapted(const Array<double>& xx, int& pos) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::update() const
    {
        ...
        do {
            val_res.set_domain(num_dom).set(index) =
                (outer_radius - ((*inner_radius_term_eq->val_t)()(num_dom))(index)) / 2. * ((*coloc[0])(index(0))) +
                (outer_radius + ((*inner_radius_term_eq->val_t)()(num_dom))(index)) / 2.;
        } while (index.inc());
        val_res.set_domain(num_dom).std_r_base();

        const int lane_count = inner_radius_term_eq->get_derivative_lane_count();
        auto mapping_derivative = [&](int lane) {
            Scalar result(sp);
            const Val_domain& radius_derivative = inner_radius_term_eq->get_der_t(lane)()(num_dom);
        ...
        if (inner_radius_term_eq->has_der_t(0))
        ...
        for (int lane = 1; lane < lane_count; ++lane)
            if (inner_radius_term_eq->has_der_t(lane))
        ...
        if (inner_radius_term_eq->has_der_t(0)) {
            Scalar der_res(sp);
            der_res.set_domain(num_dom) = -inner_radius_term_eq->get_der_t(0)()(num_dom) / 2.;
        ...
        for (int lane = 1; lane < lane_count; ++lane) {
            if (!inner_radius_term_eq->has_der_t(lane))
                continue;
            Scalar der_res(sp);
            der_res.set_domain(num_dom) = -inner_radius_term_eq->get_der_t(lane)()(num_dom) / 2.;
    ...
    void Domain_shell_inner_adapted_nosym::save(BinarySink& sink) const
    {
        ...
        center.save(sink);
        sink.write<double>(outer_radius);
        inner_radius->save(sink);
    }

    // ostream& operator<< (ostream& o, const Domain_shell_inner_adapted_nosym& so) {
    ostream& Domain_shell_inner_adapted_nosym::print(ostream& o) const
    {
    ...
    Val_domain Domain_shell_inner_adapted_nosym::der_normal(const Val_domain&, int) const
    {
        KADATH_THROW("Domain_shell_inner_adapted_nosym::der_normal not implemeted");
    }

    // Computes the cartesian coordinates
    void Domain_shell_inner_adapted_nosym::do_absol() const
    {
        for (int i = 0; i < 3; i++)
            assert(coloc[i] != nullptr);
        for (int i = 0; i < 3; i++)
        ...
        for_each_physical_point(nbr_points, [&](int i, int j, int k, std::size_t offset) {
            const double rr = (outer_radius - inner_data[offset]) / 2. * (*coloc[0])(i) +
                              (outer_radius + inner_data[offset]) / 2.;
    ...
    // Computes the cartesian coordinates
    void Domain_shell_inner_adapted_nosym::do_cart() const
    {
        for (int i = 0; i < 3; i++)
            assert(coloc[i] != nullptr);
        for (int i = 0; i < 3; i++)
    ...
    void Domain_shell_inner_adapted_nosym::do_cart_surr() const
    {
        for (int i = 0; i < 3; i++)
            assert(coloc[i] != nullptr);
        for (int i = 0; i < 3; i++)
    ...
    bool Domain_shell_inner_adapted_nosym::is_in(const Point& xx, double prec) const
    {
    ...
    const Point Domain_shell_inner_adapted_nosym::absol_to_num(const Point& abs) const
    {
        ...
        num.set(1) = 1;
        inner_radius->coef();
        double inner = inner_radius->get_base().summation(num, inner_radius->get_coef_ref());
        num.set(1) = (2. / (outer_radius - inner)) * (air - (outer_radius + inner) / 2.);

        return num;
    }

    const Point Domain_shell_inner_adapted_nosym::absol_to_num_bound(const Point& abs, int bound) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::do_coloc()
    {
        ...
        switch (type_base) {
            ...
            case LEG_TYPE:
                ...
                break;
            default:
                KADATH_THROW("Unknown type of basis in Domain_shell_inner_adapted_nosym::do_coloc");
        }
    }

    // standard base for a symetric function in z, using Chebyshev
    void Domain_shell_inner_adapted_nosym::set_cheb_base(Base_spectral& base) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::set_cheb_r_base(Base_spectral& base) const
    {
        set_cheb_base(base);
    }

    void Domain_shell_inner_adapted_nosym::set_legendre_r_base(Base_spectral& base) const
    {
        set_legendre_base(base);
    }

    // standard base for a anti-symetric function in z, using Chebyshev
    void Domain_shell_inner_adapted_nosym::set_anti_cheb_base(Base_spectral& base) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::set_cheb_base_r_spher(Base_spectral& base) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::set_cheb_base_t_spher(Base_spectral& base) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::set_cheb_base_p_spher(Base_spectral& base) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::set_legendre_base(Base_spectral& base) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::set_anti_legendre_base(Base_spectral& base) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::set_legendre_base_r_spher(Base_spectral& base) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::set_legendre_base_t_spher(Base_spectral& base) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::set_legendre_base_p_spher(Base_spectral& base) const
    {
    ...
    void Domain_shell_inner_adapted_nosym::do_der_abs_from_der_var(Val_domain** der_var, Val_domain** der_abs) const
    {
    ...
    Base_spectral Domain_shell_inner_adapted_nosym::mult(const Base_spectral& a, const Base_spectral& b) const
    {
    ...
    int Domain_shell_inner_adapted_nosym::give_place_var(char* p) const
    {
        int res = -1;
        if (strcmp(p, "R ") == 0)
            res = 0;
    ...
    void Domain_shell_inner_adapted_nosym::update_constante(const Val_domain& cor_inner_radius, const Scalar& old,
                                                      Scalar& res) const
    {
    ...
    double Domain_shell_inner_adapted_nosym::integ(const Val_domain& so, int bound) const
    {
        ...
        if (so.check_if_zero())
            ...
            switch (bound) {
                case INNER_BC: {
                    ...
                    Val_domain sint2(get_cart_surr(1) * get_cart_surr(1) +
                                     get_cart_surr(2) *
                                         get_cart_surr(2)); // sin^2 theta (should be able to get that in other manner)
                    Val_domain drdt(inner_radius->der_var(2)); // partial r / partial theta
                    Val_domain drdp(inner_radius->der_var(3)); // partial r / partial phi
                    Val_domain detq(rad * rad * (rad * rad * sint2 + sint2 * drdt * drdt + drdp * drdp));
                ...
                default:
                    cerr << "Unknown type of boundary in Domain_shell_inner_adapted_nosym::integ" << endl;
            }

            double res = 0;
            int baset = (*integrant.get_base().bases_1d[1])(0);
            Index pcf(nbr_coefs);
            // NONSYM unified COS / SIN: under collapse l_quant = j,
            //   COS:  even-j modes integrate to non-zero on [0, pi]; only
            //         j=0 picks up via M_PI * c_0 (cos(0) = 1, integrate
            //         over [0, pi] gives pi). Odd-j cos contributions
            //         cancel by symmetry.
            //   SIN:  weight 2/j per odd-j mode (zero contribution for
            //         even j; matches the SYM SIN_ODD 2/(2k+1) reindexing).
            switch (baset) {
                case COS: {
                ...
                case SIN: {
                    for (int j = 1; j < nbr_coefs(1); j++) {
                        ...
                        pcf.set(1) = j;
                        res += 2. / double(j) * val_boundary(bound, integrant, pcf);
                    }
                    break;
                }
                default:
                    KADATH_THROW("Case not yet implemented in Domain_shell_inner_adapted_nosym::integ");
            }
