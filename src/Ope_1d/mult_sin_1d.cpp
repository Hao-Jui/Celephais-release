...
namespace Kadath
{
    ...
    int mult_sin_1d_pasprevu(const double*, double*, int, int)
    {
        KADATH_THROW("mult_sin_1d not implemented.");
    }

    int mult_sin_1d_cossin(const double* src, double* dst, int nr, int stride)
    {
        assert(nr % 2 == 0);

        dst[0] = 0.5 * src[3 * stride];
        dst[stride] = 0;
        dst[2 * stride] = 0.5 * src[5 * stride];
        dst[3 * stride] = src[0] - 0.5 * src[4 * stride];
        for (int k = 4; k < nr - 2; k += 2) {
            // Cosines :
            dst[k * stride] = 0.5 * (src[(3 + k) * stride] - src[(k - 1) * stride]);
            // Sines
            dst[(k + 1) * stride] = 0.5 * (src[(k - 2) * stride] - src[(k + 2) * stride]);
        }

        dst[(nr - 2) * stride] = -0.5 * src[(nr - 3) * stride];
        dst[(nr - 1) * stride] = 0.;

        return COSSIN;
    }

    int mult_sin_1d_cos(const double* src, double* dst, int nr, int stride)
    {
        dst[0] = 0.;
    ...
    int mult_sin_1d_sin(const double* src, double* dst, int nr, int stride)
    {
    ...
    int mult_sin_1d_cos_even(const double* src, double* dst, int nr, int stride)
    {
    ...
    int mult_sin_1d_cos_odd(const double* src, double* dst, int nr, int stride)
    {
        dst[0] = 0.;
        for (int k = 1; k < nr; k++)
            // Sines
    ...
    int mult_sin_1d_sin_even(const double* src, double* dst, int nr, int stride)
    {
        dst[0] = 0.5 * src[stride];
        for (int k = 1; k < nr - 1; k++)
            // Sines
    ...
    int mult_sin_1d_sin_odd(const double* src, double* dst, int nr, int stride)
    {
        dst[0] = 0.5 * src[0];
        for (int k = 1; k < nr; k++)
            // Sines
    ...
    int mult_sin_1d(int base, const double* src, double* dst, int nr, int stride)
    {
        static int (*mult_sin_1d[NBR_MAX_BASE])(const double*, double*, int, int);
        ...
        return mult_sin_1d[base](src, dst, nr, stride);
    }
} // namespace Kadath
