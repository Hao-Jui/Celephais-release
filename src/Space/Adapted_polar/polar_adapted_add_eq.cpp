...
namespace Kadath
{
    ...
    void Space_polar_adapted::add_eq(System_of_eqs& sys, const char* eq, const char* rac, const char* rac_der,
    ...
    void Space_polar_adapted::add_eq_int_inner(System_of_eqs& sys, const char* nom)
    {
        const Domain_polar_shell_inner_adapted* pshell =
            dynamic_cast<const Domain_polar_shell_inner_adapted*>(domains[2]);
        if (pshell == nullptr) {
            KADATH_THROW("add_eq_int_inner requires that domain 2 is an inner adapted shell");
        }
        int dom = 2;
