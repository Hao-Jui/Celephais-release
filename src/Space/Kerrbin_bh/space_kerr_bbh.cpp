...
namespace Kadath
{
    ...
    Space_Kerr_bbh::Space_Kerr_bbh(int ttype, double dist, const std::vector<double>& BH1_bounds,
                                   const std::vector<double>& BH2_bounds, const std::vector<double>& outer_bounds,
                                   int nr)
    {
        ...
        auto gen_bh_domains = [&](const int nuc_i, auto& bounds, const double& eta) {
            ...
            domains[nuc_i + 1] =
                new Domain_shell_outer_adapted(*this, nuc_i + 1, ttype, bounds[RIN], bounds[RMID], center, res);
            domains[nuc_i + 2] =
                new Domain_shell_inner_adapted(*this, nuc_i + 2, ttype, bounds[RMID], bounds[RMID + 1], center, res);
        ...
        const Domain_shell_outer_adapted* pouter_1 = dynamic_cast<const Domain_shell_outer_adapted*>(domains[BH1 + 1]);
        pouter_1->vars_to_terms();
        pouter_1->update();
        const Domain_shell_inner_adapted* pinner_1 = dynamic_cast<const Domain_shell_inner_adapted*>(domains[BH1 + 2]);
        pinner_1->vars_to_terms();
        pinner_1->update();
        const Domain_shell_outer_adapted* pouter_2 = dynamic_cast<const Domain_shell_outer_adapted*>(domains[BH2 + 1]);
        pouter_2->vars_to_terms();
        pouter_2->update();
        const Domain_shell_inner_adapted* pinner_2 = dynamic_cast<const Domain_shell_inner_adapted*>(domains[BH2 + 2]);
        pinner_2->vars_to_terms();
        pinner_2->update();
    }

    Space_Kerr_bbh::Space_Kerr_bbh(BinarySource& source)
    {
        ...
        domains = new Domain*[nbr_domains];
        // First BH :
        domains[BH1] = new Domain_nucleus(BH1, source);
        domains[BH1 + 1] = new Domain_shell_outer_adapted(*this, BH1 + 1, source);
        domains[BH1 + 2] = new Domain_shell_inner_adapted(*this, BH1 + 2, source);

        for (int i = 0; i < n_shells1; ++i)
            domains[BH1 + 3 + i] = new Domain_shell(BH1 + 3 + i, source);

        // second BH :
        domains[BH2] = new Domain_nucleus(BH2, source);
        domains[BH2 + 1] = new Domain_shell_outer_adapted(*this, BH2 + 1, source);
        domains[BH2 + 2] = new Domain_shell_inner_adapted(*this, BH2 + 2, source);

        for (int i = 0; i < n_shells2; ++i)
            domains[BH2 + 3 + i] = new Domain_shell(BH2 + 3 + i, source);
    ...
    void Space_Kerr_bbh::save(BinarySink& sink) const
    {
    ...
    int Space_Kerr_bbh::nbr_unknowns_from_variable_domains() const
    {
        return (domains[BH1 + 2]->nbr_unknowns_from_adapted() + domains[BH2 + 2]->nbr_unknowns_from_adapted());
    }

    void Space_Kerr_bbh::affecte_coef_to_variable_domains(int& pos, int cc, Array<int>& zedoms) const
    {
        // In BH 1 ?
    ...
    void Space_Kerr_bbh::xx_to_ders_variable_domains(const Array<double>& xx, int& conte) const
    {
        // BH 1
        int old_conte = conte;
    ...
    void Space_Kerr_bbh::xx_to_vars_variable_domains(System_of_eqs* sys, const Array<double>& xx, int& pos) const
    {
        ...
        for (int i = 0; i < sys->ncst; i++)
            if (sys->cst[i * (sys->dom_max - sys->dom_min + 1)]->get_type_data() == TERM_T)
                for (int n = 0; n < sys->cst[i * (sys->dom_max - sys->dom_min + 1)]->val_t->get_n_comp(); n++) {
                    ...
                    so = 0;
                    so.set_domain(BH1 + 1) =
                        (*sys->cst[i * (sys->dom_max - sys->dom_min + 1) + BH1 + 1]->val_t->cmp[n])(BH1 + 1);
                    so.set_domain(BH1 + 2) =
                        (*sys->cst[i * (sys->dom_max - sys->dom_min + 1) + BH1 + 2]->val_t->cmp[n])(BH1 + 2);
                    so.set_domain(BH2 + 1) =
                        (*sys->cst[i * (sys->dom_max - sys->dom_min + 1) + BH2 + 1]->val_t->cmp[n])(BH2 + 1);
                    so.set_domain(BH2 + 2) =
                        (*sys->cst[i * (sys->dom_max - sys->dom_min + 1) + BH2 + 2]->val_t->cmp[n])(BH2 + 2);
                    ...
                    sys->cst[i * (sys->dom_max - sys->dom_min + 1) + BH1 + 1]->val_t->cmp[n]->set_domain(BH1 + 1) =
                        res(BH1 + 1);
                    sys->cst[i * (sys->dom_max - sys->dom_min + 1) + BH1 + 2]->val_t->cmp[n]->set_domain(BH1 + 2) =
                        res(BH1 + 2);
                    sys->cst[i * (sys->dom_max - sys->dom_min + 1) + BH2 + 1]->val_t->cmp[n]->set_domain(BH2 + 1) =
                        res(BH2 + 1);
                    sys->cst[i * (sys->dom_max - sys->dom_min + 1) + BH2 + 2]->val_t->cmp[n]->set_domain(BH2 + 2) =
                        res(BH2 + 2);
                }
    ...
    Array<int> Space_Kerr_bbh::get_indices_matching_non_std(int dom, int bound) const
    {
        if (dom == BH1 + n_shells1 + 2) {
            ...
            switch (bound) {
                case OUTER_BC:
                    ...
                    break;
                default:
                    KADATH_THROW("Bad bound in Space_Kerr_bbh::get_indices_matching_non_std");
            }
            return res;
        }
        ...
        KADATH_THROW("Bad domain in Space_Kerr_bbh::get_indices_matching_non_std");
    }
