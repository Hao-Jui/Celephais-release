...
namespace Kadath
{
    namespace
    {
        std::vector<double> to_vector(const Array<double>& bounds)
        {
            assert(bounds.get_ndim() == 1);
            std::vector<double> result(bounds.get_size(0));
            for (int i = 0; i < bounds.get_size(0); ++i)
                result[i] = bounds(i);
            return result;
        }
    } // namespace

    Space_spheric_homothetic::Space_spheric_homothetic(int ttype, const Point& center, const Dim_array& res,
                                                       const Array<double>& bounds)
        : Space_spheric_homothetic(ttype, center, res, to_vector(bounds))
    {
    }
                                                       ...
                                                       const std::vector<double>& bounds)
    {
        ...
        const Domain_shell_outer_homothetic* pshell_outer =
            dynamic_cast<const Domain_shell_outer_homothetic*>(domains[1]);
        pshell_outer->vars_to_terms();
        pshell_outer->update();
        const Domain_shell_inner_homothetic* pshell_inner =
            dynamic_cast<const Domain_shell_inner_homothetic*>(domains[2]);
        pshell_inner->vars_to_terms();
        pshell_inner->update();
    }

    Space_spheric_homothetic::Space_spheric_homothetic(BinarySource& source)
    {
        ...
        domains[1] = new Domain_shell_outer_homothetic(*this, 1, source);
        domains[2] = new Domain_shell_inner_homothetic(*this, 2, source);
        for (int i = 3; i < nbr_domains - 1; ++i)
    ...
    Space_spheric_homothetic::~Space_spheric_homothetic()
    {
        const Domain_shell_outer_homothetic* pshell_outer =
            dynamic_cast<const Domain_shell_outer_homothetic*>(domains[1]);
    ...
    void Space_spheric_homothetic::save(BinarySink& sink) const
    {
    ...
    int Space_spheric_homothetic::nbr_unknowns_from_variable_domains() const
    {
        return domains[1]->nbr_unknowns_from_adapted();
    }

    void Space_spheric_homothetic::affecte_coef_to_variable_domains(int& pos, int cc, Array<int>& zedoms) const
    {
    ...
    void Space_spheric_homothetic::xx_to_ders_variable_domains(const Array<double>& xx, int& conte) const
    {
        int old_conte = conte;
    ...
    void Space_spheric_homothetic::xx_to_vars_variable_domains(System_of_eqs* sys, const Array<double>& xx,
                                                               int& pos) const
    {
        int old_pos = pos;
    ...
    Array<int> Space_spheric_homothetic::get_indices_matching_non_std(int dom, int bound) const
    {
    ...
    void Space_spheric_homothetic::add_eq_ori(System_of_eqs& sys, const char* name)
    {
