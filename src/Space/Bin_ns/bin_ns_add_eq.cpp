...
namespace Kadath
{
    ...
    // The star-side block is sym-specific (inner shells come AFTER the adapted
    // pair and the bispheric matching sits at ADAPTED + 1 + n_shells); the nosym
    // class orders the shells before the adapted pair on purpose.
    void Space_bin_ns::add_eq(System_of_eqs& sys, const char* eq, const char* rac, const char* rac_der, int nused,
                              Array<int>** pused)
    {
        ...
        // Post-adapted outer shells: each shell-to-next seam (fixed radius).
        for (int i = 0; i < n_shells1; ++i) {
            match_radial_seam(ADAPTED1 + 1 + i);
            sys.add_eq_inside(ADAPTED1 + 2 + i, eq, nused, pused, rac);
        }

        // Matching with bispheric (always import: spherical vs bipolar charts).
    ...
    void Space_bin_ns::add_eq_nozec(System_of_eqs& sys, const char* eq, const char* rac, const char* rac_der, int nused,
                                    Array<int>** pused)
    {
        BinaryCoSpaceEquations::bin_ns_add_eq_nozec(*this, sys, eq, rac, rac_der, nused, pused);
    }

    void Space_bin_ns::add_eq_nozec(System_of_eqs& sys, const char* eq, const char* rac, const char* rac_der,
                                    const List_comp& used)
    {
        add_eq_nozec(sys, eq, rac, rac_der, used.get_ncomp(), used.get_pcomp());
    }

    // Sym-specific (the nosym class indexes the star blocks through
    // NS + n_shells on purpose).
    void Space_bin_ns::add_eq_noshell(System_of_eqs& sys, const char* eq, const char* rac, const char* rac_der,
                                      int nused, Array<int>** pused)
    {

        // First NS (outer shells are excluded from the computational space)
        sys.add_eq_inside(NS1, eq, nused, pused, rac);
        ...
        // Matching with bispheric :
        sys.add_eq_matching_import(ADAPTED1 + 1, OUTER_BC, rac, nused, pused, rac, true);
        sys.add_eq_matching_import(OUTER, INNER_BC, rac_der, nused, pused, rac, true);
        sys.add_eq_matching_import(OUTER + 1, INNER_BC, rac_der, nused, pused, rac, true);

        // Second NS (outer shells are excluded from the computational space)
        sys.add_eq_inside(NS2, eq, nused, pused, rac);
        ...
        // Matching with bispheric :
        sys.add_eq_matching_import(ADAPTED2 + 1, OUTER_BC, rac, nused, pused, rac, true);
        sys.add_eq_matching_import(OUTER + 3, INNER_BC, rac_der, nused, pused, rac, true);
        sys.add_eq_matching_import(OUTER + 4, INNER_BC, rac_der, nused, pused, rac, true);
    ...
    {
        BinaryCoSpaceEquations::add_eq_int_inf<Domain_compact>(*this, sys, nom);
    }
