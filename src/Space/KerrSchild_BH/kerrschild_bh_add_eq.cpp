...
namespace Kadath
{
    ...
    void Space_KerrSchild_bh::add_eq(System_of_eqs& sys, const char* eq, const char* rac, const char* rac_der,
    ...
    void Space_KerrSchild_bh::add_eq_int_bh(System_of_eqs& sys, const char* nom)
    {
        ...
        if (!indic) {
            ...
            if (indic) {
                ...
                sys.eq_int[sys.neq_int]->set_part(
                    0, new Ope_sub(&sys, sys.give_ope(2, p1, INNER_BC), sys.give_ope(2, p2, INNER_BC)));
            }
        }
        sys.neq_int++;
        sys.nbr_conditions = -1;
