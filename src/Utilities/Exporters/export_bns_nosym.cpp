...
std::array<std::vector<double>, NUM_OUT> KadathExportBNSNosym(int const npoints, double const* xx, double const* yy,
                                                              double const* zz, char const* fn)
{
    ...
    if (eos_type == "Cold_Table") {
        ...
        throw std::invalid_argument("Unsupported EOS type for BNS no-sym export: " + eos_type);
    }
    ...
    syst.add_cst("angs1", ang1 * M_PI / 180.0);
    syst.add_cst("angs2", ang2 * M_PI / 180.0);
