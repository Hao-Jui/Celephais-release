#!/bin/bash
# test_bh2d_smoke.sh — BH2d infrastructure smoke test.
...
if [ ! -x "$APP_DIR/bin/Release/solve" ]; then
    echo "SKIP: BH2d solve binary missing: $APP_DIR/bin/Release/solve"
    exit 0
fi
...
echo "Running BH2d smoke test..."
STDOUT="$(mktemp)"
cd "$WORK_DIR"
SMOKE_TIMEOUT="${BH2D_SMOKE_TIMEOUT:-60}"
SOLVER_RC=0
...
if [ "$SOLVER_RC" -ne 0 ] && [ "$SOLVER_RC" -ne 124 ]; then
    echo "FAIL: BH2d crashed (exit $SOLVER_RC)"
    tail -20 "$STDOUT" >&2
    rm -f "$STDOUT"
    exit 1
fi
...
if [ ! -s "$STDOUT" ]; then
    echo "FAIL: BH2d produced no output"
    rm -f "$STDOUT"
    exit 1
fi
...
rm -f "$STDOUT"
echo "PASS: BH2d smoke (launched, no crash, no sanitizer)"
exit 0
