...
namespace Kadath
{
    ...
    class Space_bhns : public Space
    {
      ...
      public:
        ...
        /**
         * Per-domain-resolution variant: each domain carries its own
         * \c Dim_array (radial, theta and phi point counts), used by per-domain
         * AMR p-refinement (including the bispheric phi block). \c res_per_domain
         * is indexed by absolute domain index and must have one entry per domain.
         * The five bispheric domains (\c OUTER .. \c OUTER+4) must share an
         * identical \c Dim_array, and the NS adapted pair and BH homothetic pair
         * must each agree in theta and phi (each shares one surface object).
         */
        Space_bhns(int ttype, double dist, const std::vector<double>& NS_bounds, const std::vector<double>& BH_bounds,
                   const std::vector<double>& outer_bounds, const std::vector<Dim_array>& res_per_domain);

        Space_bhns(BinarySource& source, bool oldspace = false); ///< Modern API.

        ~Space_bhns() override; ///< Destructor
