...
{
namespace xy_snapshot_detail
{
// Collocation points whose |z| falls below this threshold are treated as
// lying in the xy-plane. The threshold is generous because collocation nodes
// are not guaranteed to land exactly on z == 0 in every domain.
constexpr double xy_plane_tolerance = 1e-10;

inline std::filesystem::path xy_snapshot_output_path()
{
    const char* home_celephais = std::getenv("HOME_CELEPHAIS");
    const std::filesystem::path repo_root = home_celephais != nullptr
        ? std::filesystem::path{home_celephais}
        : std::filesystem::current_path();
    const std::filesystem::path output_dir = repo_root / "matlab" / "snapshot";
    std::filesystem::create_directories(output_dir);

    return output_dir / "xy_snapshot.dat";
}
...
// xy-plane collocation point, the columns:
...
int xy_snapshot_binary_main(int argc, char** argv)
{
    using namespace Kadath;
    using xy_snapshot_detail::xy_plane_tolerance;
    ...
    return KadathApps::guarded_run([&] {
        if (argc < 2) {
            KADATH_THROW(
                "Usage: ./xy_snapshot /<path>/<ID base name>.toml|.dat [xy|all]  "
                "e.g. ./xy_snapshot converged.toml xy");
        }

        const bool xy_only = (argc < 3) || (std::string{argv[2]} != "all");
        ...
        Vector velocity_cov =
            xy_snapshot_detail::binary_sacra_velocity_cov(space, bconfig, basis, conf, logh, phi);

        const std::filesystem::path output_path = xy_snapshot_detail::xy_snapshot_output_path();

        std::ofstream out{output_path};
        if (!out) {
            KADATH_THROW("xy_snapshot: cannot open output file: " + output_path.string());
        }

        std::cout << "xy_snapshot: writing to " << output_path << "\n";
        ...
        auto write_domains = [&]<class eos_t>() {
            for (int d = 0; d < ndom; ++d) {
                ...
                do {
                    ...
                    if (xy_only && std::fabs(z) > xy_plane_tolerance) {
                        continue;
                    }
        ...
        if (eos_type == "Cold_PWPoly") {
            ...
            KADATH_THROW("xy_snapshot: unrecognised EOS type: " + eos_type);
        }

        out.close();
        std::cout << "xy_snapshot: done.\n";
...
// identically zero. Emits, for every xy-plane collocation point, the columns:
//   dom x y z P N H rho phi bet_x bet_y bet_z ux uy uz [varscal [xiScal]]
template <typename space_t, typename FieldTraits>
int xy_snapshot_ns_main(int argc, char** argv)
{
    using namespace Kadath;
    using xy_snapshot_detail::xy_plane_tolerance;
    ...
    return KadathApps::guarded_run([&] {
        ...
        Base_tensor basis(shift.get_basis());
        Vector velocity_cov =
            xy_snapshot_detail::ns_sacra_velocity_cov(space, bconfig, basis, conf, lapse, shift, logh);
...
// emitted as zero.
template <typename space_t, typename FieldTraits>
int xy_snapshot_three_body_main(int argc, char** argv)
{
    using namespace Kadath;
    using xy_snapshot_detail::xy_plane_tolerance;
    ...
    return KadathApps::guarded_run([&] {
        ...
        Base_tensor basis(shift.get_basis());
        Vector velocity_cov = xy_snapshot_detail::zero_sacra_velocity_cov(space, basis);
