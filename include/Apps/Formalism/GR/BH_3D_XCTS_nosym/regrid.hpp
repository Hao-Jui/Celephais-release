...
#include "Apps/Seed/GR/bh_nosym_seed.hpp"
...
template <typename config_t> int bh_3d_xcts_nosym_regrid(config_t& bconfig, const int new_res, std::string outputfile)
{
    ...
    using space_t = Space_adapted_bh_nosym;
    ...
    Seed::setup_co<BH, Space_adapted_bh_nosym>(bconfig, true);
    ...
    const Domain_shell_outer_homothetic_nosym* old_outer_homothetic =
        dynamic_cast<const Domain_shell_outer_homothetic_nosym*>(old_space.get_domain(1));
