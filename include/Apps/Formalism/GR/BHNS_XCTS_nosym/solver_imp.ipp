...
// Deliberately omit bco_binary_setup.hpp (the symmetric solver_imp includes it):
// it pulls the symmetric NS/BH driver buckets, which ODR-clash with the nosym
// drivers below (same signatures). Unused here anyway.
#include "Apps/Formalism/GR/BH_3D_XCTS_nosym/solver.hpp"
...
bhns_xcts_nosym_solver<eos_t, config_t, space_t>::bhns_xcts_nosym_solver(config_t& config_in, space_t& space_in,
...
template <class eos_t, typename config_t, typename space_t>
std::string bhns_xcts_nosym_solver<eos_t, config_t, space_t>::converged_filename(const std::string& stage) const
{
    return converged_gr_bhns_nosym_filename(bconfig, space, stage);
}
...
template <class eos_t, typename config_t, typename space_t>
int bhns_xcts_nosym_solver<eos_t, config_t, space_t>::solve(bool want_warmup)
{
    int rank = 0;
...
template <class eos_t, typename config_t, typename space_t>
void bhns_xcts_nosym_solver<eos_t, config_t, space_t>::syst_init(System_of_eqs& syst)
{
    using namespace Kadath::Margherita;
    ...
    // global-rotation field + BCO1/infinity surface normals (mg, sm, einf)
    ...
    // flat space surface normal for the BCO2 (BH) side (sm/einf via helper above)
    ...
    // conformal combinations (NP, Ntilde) + ADM/Komar mass integrands at infinity
    xcts::add_conformal_combinations_and_adm_integrands(syst, ndom);
    syst.add_def("intMsq  = P^4 / 4. / 4piG");

    // thermodynamic defs from the (log) enthalpy: h, rho, eps, press, dHdlnrho, delta
    xcts::add_thermodynamic_defs(syst);

    // conformal extrinsic curvature A^ij + ADM linear-momentum integrands at infinity
    xcts::add_extrinsic_curvature_and_adm_momenta(syst, ndom);

    // quasi-local spin integrands, evaluated just outside each compact object
    xcts::add_quasilocal_spin_integrands(syst, space.ADAPTEDNS + 1, space.ADAPTEDBH + 1);

    // -------------------------------------------------------------------------
    // No-symmetry rotational-gauge closure (bordered). muz1 forces the rigid-
    // z-rotation direction mmz in the NS-interior eqbet (see the stage eqbet
    // defs); the single point condition pins curl_z(bet) = (D^i bet^j - D^j
    // bet^i) ex_i ey_j = 0 at the NS center. +1 unknown balanced by +1
    // condition keeps the Jacobian square. The BH side is excised / von-Neumann
    // and keeps the plain shift equation. Toggle via BHNS_NOSYM_ROT_GAUGE.
    // -------------------------------------------------------------------------
    if (rotational_gauge_enabled()) {
        syst.add_var("muz1", muz1_gauge);

        Index pos_center1(space.get_domain(space.NS)->get_nbr_points());
        syst.add_eq_val(space.NS, "(D^i bet^j - D^j bet^i) * ex_i * ey_j", pos_center1);
    }
}
...
template <class eos_t, typename config_t, typename space_t>
void bhns_xcts_nosym_solver<eos_t, config_t, space_t>::print_diagnostics(System_of_eqs const& syst, const int ite,
...
{
    // nosym BH-NS superposition — shared body, instantiated with the nosym
    // space + domain types (the only difference from the symmetric variant).
    bhns_setup_boosted_3d_impl<eos_t, Space_spheric_adapted_nosym, Space_adapted_bh_nosym, Space_bhns_nosym,
                               Domain_shell_outer_adapted_nosym, Domain_shell_inner_adapted_nosym,
                               Domain_shell_outer_homothetic_nosym>(NSconfig, BHconfig, bconfig);
}
