...
using sacra_export::LevelRange;
using sacra_export::compute_level_range;
using sacra_export::snap_to_parent_grid;
...
int main(int argc, char** argv) {
    ...
    syst.add_cst("angs1", bconfig(DEG, BCO1) * M_PI / 180.0);
    syst.add_cst("omes2", bconfig(OMEGA, BCO2));
    syst.add_cst("angs2", bconfig(DEG, BCO2) * M_PI / 180.0);
    ...
    for (int lv = 0; lv <= lvmax; lv++) {
        ...
        if (rank == 0) {
            ...
            auto axis_line = [&](const char* name, double lo, double hi, int lo_idx, int hi_idx) {
                fout << "   " << name << ":" << eWpDe3(lo, 21, 12) << eWpDe3(hi, 21, 12) << "  "
        ...
        for (int lg = lr.first_lg; lg <= lr.last_lg; lg++) {
            ...
            for (int kg = kd_org - 6; kg <= ku_org + 6; kg++) {
                ...
                for (int jg = jd_org - 6; jg <= ju_org + 6; jg++) {
                    ...
                    //ALPHA, BX,BY,BZ, phitmp, EK, AKXX..AKYZ, rho, ux, uy, uz
                    // The spectral rho(h) evaluation is meaningless outside the
                    // stars (h <= 1); zero it there like the BNS_nosym exporter.
