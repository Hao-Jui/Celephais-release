...
int main(int argc, char** argv) {
  ...
  if(argc < 3) {
    ...
    [[maybe_unused]] bool const is_ns_obj1(true) ; //true: object 1 is NS, false: object 1 is BH
    ...
    int const idist(16);
    if ( idist > pow(2., depth_max1 - flvmax ) ) {
      cerr << "AMR structure will fail: idist < 2^n / 4 must hold, where n = moving levels for NS\n"
	   << "please reduce idist or increase moving levels" << endl;
      MPI_Abort(MPI_COMM_WORLD, rc);
    }
    ...
    if (nb_procs > lvmax0*total_nb_lg) {
      cerr << "The number of procs should be <= lvmax0*total_nb_lg = " << lvmax0*total_nb_lg << "." << endl;
      MPI_Abort(MPI_COMM_WORLD, rc);
    }
    ...
    ts_dx[lvmax] = bconfig(DIST) / ( idist * ( ju_org - jd_org + cell_cent_shift ) ) ;

    cout << setprecision(15) ;
    if(rank==0) cout << "ts_dx[lvmax] = " << ts_dx[lvmax] << endl ;
    ...
    // SACRA uses evolution scaling \tilde{A}_ij = \Psi^{-4} A = \Psi^{-6} \hat{A}
    ...
    for(int lv(0) ; lv <= lvmax ; lv++) {
        ...
        if(lv <= flvmax) { // grid centered on origin (0,0,0)
            ...
            if(rank==0)
                cout << "Rank " << rank << ": center_auxi(1) = " << center_auxi(1)
		     << ", origin_coord(1) = " << origin_coord(1) << ", ts_dx[p_lv] = " << ts_dx[p_lv]
		     << ", so positive_ratio_step = " << positive_ratio_step << ", so " << flush ;
        ...
        if(rank==0) {
            cout << "Rank " << rank << " writing top info for level " << lv << endl << endl << endl ;
    ...
    for(int lv(first_level) ; lv <= last_level ; lv++) {
        ...
        for(int lg(first_lg) ; lg <= last_lg ; lg++) { // "g"lobal indices on current level
            cout << "Rank " << rank << " currently writing fields data at level " << lv
		 << " from lg = " << first_lg << " to lg = " << last_lg << ", both included." << endl ;
            ...
            for(int kg(kd_org-6) ; kg <= ku_org+6 ; kg++) {
                current_point.set(2) = ts_yd[lv] + ts_dy[lv]*(kg-kd_org) + COMy ; // WE ACTUALLY EXPORT VALUE AT (COM, COMY,0) IN FUKA CONFIG .dat)
			...
			for (int ff(0) ; ff < static_cast<int>(field.size()) ; ff++)
	...
	double BH_2_ahmass(bconfig(MCH    , BCO2)) ;
        ...
        output_file_path = output_dir_path + "concatenate.sh";
	    ...
	    // IN THIS CASE, first_lg = ld_org-6 AND last_lg = lu_org+6 FOR ALL rank, INCLUDING rank 0
        } else {
